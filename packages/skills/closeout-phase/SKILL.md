...
2. Run `make-docs operations work-phase-state TARGET_PHASE --json` on the target phase before manually reading phase details.
3. Run `make-docs operations closeout-probe --repo-root . --scope auto --json` for changed files, contracts, candidate coordinates, risk/history hints, and validation hints.
