...
Use this reference only when the assignment needs saved lifecycle progress or resumes an existing run. It does not activate another skill. Short reviews need no saved run.

## Keep the Existing Boundary

Use supported Make Docs lifecycle operations. Let Make Docs resolve the normal Store path. Let the CLI resolve the project identity from its supported configuration and global Store. Do not create installation records, update Make Docs, write SQL, or add a state directory to make optional capture work.
...
Use the installed CLI help and the applicable project instructions if a command or boundary is unclear. Follow the current assignment and approval scope for any repair or change to Make Docs. A missing capability is not permission to change the installation.
...
## Capability and Failure Limits

Check the available CLI or tool schema in the current environment. Past checks in another repository do not prove present capability. If the CLI is absent or optional capture fails, report that no state was recorded and continue ordinary work that does not require it. Do not write directly to the Store, queue a future capture, or create local operational state.

Installation, upgrade, migration, and other managed CLI writes require their Store records. Those operations must stop safely when the required records cannot be saved. Optional lifecycle capture does not weaken that rule.
