...
- Use `.make-docs/prompts/system/` for prompts.
...
- Keep project state in `.make-docs/manifest.json`, `.make-docs/conflicts/`, and project config. General lifecycle run state lives in the machine Store at `~/.make-docs/`. Do not copy runtime state into `docs/assets/`.
