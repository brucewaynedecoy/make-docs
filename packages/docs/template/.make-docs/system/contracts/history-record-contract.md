...
See `.make-docs/system/references/wave-model.md` for W/R/P semantics and general resolution rules. The rules below are specific to breadcrumb records and extend (do not replace) the general model.
...
- Apply `.make-docs/system/references/path-and-link-hygiene.md` for repo-relative path rules, sanitized placeholders, and full-path exceptions.
