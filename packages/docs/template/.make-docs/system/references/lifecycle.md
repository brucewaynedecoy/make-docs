...
| Review | Inspect the actual human-facing surface for `direct` impact when it exists. Tie `indirect` evidence to the stated human effect. The agent prepares and records a concise review with per-promise observations, conclusions, limits, and next actions. |
| Coverage | Apply Human Experience Review as an agent-owned lens over suitable evidence for each applicable promise. Reuse evidence that answers the promise. Human Experience Review is not a separate testing type. |
| Acceptance | Keep claims within the agent review. For completed direct human-facing work, normally give a short optional experience handoff. A missing human response does not block closure or create an obligation unless accepted authority explicitly defines a human acceptance gate. Keep a material gap open or route later owed work through an accepted obligation. |
