...
Before writing anything, read `.make-docs/system/contracts/coverage-pass-contract.md`, `.make-docs/system/contracts/guide-contract.md`, the matching guide template in `.make-docs/system/templates/`, and the router in `docs/assets/library/`. Treat those files as the authority for coverage-pass mechanics, audience intent, frontmatter, slug rules, guide coverage decisions, and future coverage handling.
