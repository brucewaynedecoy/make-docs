...
Then generate a dated delta backlog in `docs/work/`. Follow the instructions, references, and templates in the `docs` directory, especially `docs/.references/prd-change-management.md`. The backlog should stay scoped to the requested change, remain dependency-ordered, and cite both the new change docs and the affected baseline docs in each phase's source-traceability section.
