...
import { acquireStoreAccess, CURRENT_STORE_SCHEMA_VERSION, formatStoreIssue, makeStoreIssue, openStoreDatabase, recoverDeadStoreAccessSessions, StoreUnavailableError } from '../src/store/database';
...
describe('Store-owned installation safety',()=>{
  ...
  it('does not claim resume is safe when the complete plan was not recorded',()=>{expect(()=>operation(()=>{replace('note.md','x');throw new Error('fault');})).toThrow();const id=pendingId();expect(recoverInstallationOperation(project,id,'resume',false,store).status).toBe('blocked');const status=readInstallationStatus(project,store);expect(status.status).toBe('recovery-required');expect(status.nextAction).toContain('--rollback --dry-run');expect(status.nextAction).not.toContain('--resume');});
  ...
  it('keeps shared Store sessions safe under 20 processes and five checkouts',async()=>{
    ...
    const children=Array.from({length:20},(_,index)=>{const root=roots[index%roots.length];const file=`result-${index}.txt`;const script=`import {withInstallationOperation,recordPlannedFileChange} from './packages/cli/src/store/installation-state.ts';import{writeFileSync}from'node:fs';const wait=new Int32Array(new SharedArrayBuffer(4));for(let attempt=1;attempt<=3;attempt++){try{withInstallationOperation(${JSON.stringify(root)},'stress',()=>recordPlannedFileChange(${JSON.stringify(root)},${JSON.stringify(file)},{kind:'file',content:'done'},()=>writeFileSync(${JSON.stringify(path.join(root,file))},'done')),{storeRoot:${JSON.stringify(store)}});break;}catch(error){if(error?.code!=='store-unavailable'||error?.issue?.code!=='contention-timeout'||error?.issue?.retryable!==true||attempt===3)throw error;Atomics.wait(wait,0,0,attempt*100);}}`;return spawn(process.execPath,['--import','tsx','--input-type=module','-e',script],{cwd:path.resolve('../..'),stdio:['ignore','pipe','pipe']});});
  ...
  it('rebinds an unchanged moved directory without durable device or inode fields',()=>{operation(()=>replace('note.md','content'));const before=getInstallationCheckoutId(project,store);withInstallationDatabase(project,db=>{const columns=(db.prepare('PRAGMA table_info(installation_checkouts)').all() as Array<{name:string}>).map(row=>row.name);expect(columns).not.toContain('root_device');expect(columns).not.toContain('root_inode');},{storeRoot:store,readOnly:true});const moved=path.join(temp,'moved');renameSync(project,moved);withInstallationOperation(moved,'move',()=>{}, {storeRoot:store});expect(getInstallationCheckoutId(moved,store)).toBe(before);});
  it('preserves a moved checkout claim when verified content changed',()=>{operation(()=>replace('note.md','content'));const before=getInstallationCheckoutId(project,store);const moved=path.join(temp,'moved');renameSync(project,moved);writeFileSync(path.join(moved,'note.md'),'changed after move');expect(()=>withInstallationOperation(moved,'move',()=>{}, {storeRoot:store})).toThrow(/last verified Store facts/);withInstallationDatabase(moved,db=>{const row=db.prepare('SELECT root_path FROM installation_checkouts WHERE checkout_id=?').get(before) as {root_path:string};expect(row.root_path).toBe(canonicalInstallationPath(project));},{storeRoot:store,readOnly:true});expect(readFileSync(path.join(moved,'note.md'),'utf8')).toBe('changed after move');});
