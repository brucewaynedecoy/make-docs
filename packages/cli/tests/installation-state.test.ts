...
describe('Store-owned installation safety',()=>{
  ...
  it('rebinds an unchanged moved directory without using durable device or inode identity',()=>{operation(()=>replace('note.md','content'));const before=getInstallationCheckoutId(project,store);withInstallationDatabase(project,db=>db.prepare("UPDATE installation_checkouts SET root_device='legacy-device',root_inode='legacy-inode'").run(),{storeRoot:store});const moved=path.join(temp,'moved');renameSync(project,moved);withInstallationOperation(moved,'move',()=>{}, {storeRoot:store});expect(getInstallationCheckoutId(moved,store)).toBe(before);});
