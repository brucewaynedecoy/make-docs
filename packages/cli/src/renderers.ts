...
function renderRootInstruction(): string {
  const body = [
...
function renderDesignWorkflow(profile: InstallProfile): string {
  ...
  if (profile.capabilityState.plans.effectiveSelection) {
    return [
      ...
      "  - `baseline-plan`",
      "  - `change-plan`",
      "- Planning prompt links are unavailable in this profile because the required planning assets are not installed locally.",
      "- Record:",
      "  - `Next Step:` rerun `npx @brucewaynedecoy/make-docs@next reconfigure` and enable `plans` if you want local planning prompts",
  ...
  return [
    ...
    "",
    "This workflow ends with design docs in `docs/designs/` using the form `docs/designs/YYYY-MM-DD-<slug>.md`. Local planning assets are not installed in this profile.",
    "",
    "## Preflight",
    "",
    "Before writing:",
    ...
    "  - `Next Step:` rerun `npx @brucewaynedecoy/make-docs@next reconfigure` and enable `plans`",
...
function renderDesignContract(profile: InstallProfile): string {
  ...
  if (profile.capabilityState.plans.effectiveSelection) {
    return [
      ...
      "",
      "Do not add a `Next Prompt` link when the required planning prompts are not installed locally.",
      "",
      ...
      "- If the required planning prompts are not installed locally, do not create broken prompt links.",
      "",
    ].join("\n");
  }
  ...
  return [
    ...
    "",
    "- `Route:` `planning-not-installed`",
    "- `Next Step:` rerun `npx @brucewaynedecoy/make-docs@next reconfigure` and enable `plans` if you want local planning prompts and templates",
...
function renderDesignTemplate(profile: InstallProfile): string {
  ...
  return [
    ...
    "",
    "- Route: planning-not-installed",
    "- Next Step: Re-run `npx @brucewaynedecoy/make-docs@next reconfigure` and enable `plans` to install local planning prompts and templates.",
