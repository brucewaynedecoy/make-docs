...
import { PROJECT_READ_ACCESS, STORE_WRITE_PROJECT_READ_ACCESS, STORE_WRITE_PROJECT_WRITE_ACCESS } from '../access';
...
export const projectLayoutOperations: OperationDefinition[] = [
    {id: 'project.persona.list', summary: 'Read the effective project audiences without the Store.', mutates: 'read', access: PROJECT_READ_ACCESS, status: 'active', inputSchema: listInput,
        handler(raw, context) {const input = listInput.parse(raw); return listProjectPersonas(path.resolve(input.targetRoot ?? context.cwd));}},
    {id: 'project.layout.preview', summary: 'Preview exact legacy file destinations, links and blockers without writes.', mutates: 'read', access: PROJECT_READ_ACCESS, status: 'active', inputSchema: previewInput,
    ...
    {id: 'project.layout.prepare', summary: 'Save a reviewed CLI or manual layout plan in the Store before file changes.', mutates: 'write', access: STORE_WRITE_PROJECT_READ_ACCESS, status: 'active', inputSchema: prepareInput,
    ...
    ...(['apply', 'verify'] as const).map(action => ({id: `project.layout.${action}`, summary: action === 'apply' ? 'Apply and verify one prepared CLI layout operation.' : 'Check a prepared manual layout and record verified completion.', mutates: 'write' as const, access: action === 'apply' ? STORE_WRITE_PROJECT_WRITE_ACCESS : STORE_WRITE_PROJECT_READ_ACCESS, status: 'active' as const, inputSchema: operationInput,
