...
- **PRD 32 preserved byte-identically**: store lifecycle handling changed none of the repo-level backup behavior — backups stay under `.make-docs/backup/**`, legacy root `.backup/**` stays protected, agentics pruning rules are untouched. The store code runs strictly after the PRD 32-governed removal loop and writes only under the store root; `tests/uninstall.test.ts`, `tests/backup.test.ts`, and `tests/audit.test.ts` are the unmodified baseline proof, and `tests/store-lifecycle.test.ts` additionally asserts both protected backup roots survive an uninstall with store handling active.
