...
| `store.db.bridge-v<source>-to-v5-<uuid>.backup` | Verified, private pre-conversion Store backup. P5 retains it until a separate cleanup approval permits deletion. |
...
### SQL schema and table layout (introduced in schema version 1)
...
- **R-LIFE-3 (update migration + pre-v2 detection)**: setup and update use the reviewed Store compatibility bridge before any schema write. Ordinary Store writers reject schemas below 5 and do not apply a migration. The bridge classifies the Store without writes, lists the exact Store changes and blockers, and then uses the shared operation journal for an approved conversion. Pre-v2 project configuration detection remains in `classifyCompatibilityState` (`packages/cli/src/compatibility.ts`): schema-version-1 manifests classify as `clean-v1`/`modified-v1`, and `guardCompatibilityDisposition` presents the warning-and-backup-or-cancel flow.
