...
| Validation | `tsc --noEmit` clean, `npm run build -w starter-docs` succeeds (tsup bundles to `dist/index.js`; new modules will be pulled into the bundle when Phase 3+ imports them), all 44 existing tests pass. Unit tests for the loader and resolver are scheduled for Phase 7. |
