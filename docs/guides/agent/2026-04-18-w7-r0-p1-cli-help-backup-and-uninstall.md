...
| Area | Summary |
| --- | --- |
| CLI surface | [`packages/cli/src/cli.ts`](../../../packages/cli/src/cli.ts) now recognizes `backup` and `uninstall`, parses `--permissions` and `--backup`, rejects invalid cross-command flag mixes, and keeps implicit command inference limited to install and update flows. |
