...
| CLI routing | [`packages/cli/src/cli.ts`](../../../packages/cli/src/cli.ts) now dispatches `starter-docs uninstall` through the real uninstall executor, forwards `--backup`, `--permissions`, and `--target`, defaults uninstall permissions to `confirm`, and keeps `uninstall --help` on the command help surface. |
...
| Uninstall tests | [`packages/cli/tests/uninstall.test.ts`](../../../packages/cli/tests/uninstall.test.ts) covers `allow-all` removal, modified root instruction preservation, unmanaged descendant preservation, confirm-mode cancellation without mutation, `uninstall --backup` audit reuse, and partial delete failure reporting. |
