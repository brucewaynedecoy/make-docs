...
| Reconfigure guard | [`packages/cli/src/cli.ts`](../../../packages/cli/src/cli.ts) keeps `starter-docs reconfigure` manifest-only, opens the wizard from saved selections interactively, and now rejects `starter-docs reconfigure --yes` unless at least one selection flag is provided. |
...
| Verification | `npm test -w starter-docs -- tests/cli.test.ts`, `npm test -w starter-docs`, and `npm run build -w starter-docs` passed after the Phase 2 changes. |
