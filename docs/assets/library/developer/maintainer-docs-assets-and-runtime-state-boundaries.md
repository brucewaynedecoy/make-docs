...
- Blocked by: the W18 R7 store-seam consumer. Update when: W18 R7 run-state storage relocates the Playbook runner's `.make-docs/runs/playbooks/**` writes onto the `playbook_runs` seam. Guide change: fold the Playbook run-state relocation into the runtime-state story once nothing current writes under `.make-docs/runs/` (including retiring the tool-directory runtime-state classification caveat). The former tool-level-commands half of this bullet is resolved: `uninstall` and `update` shipped in W18 R11 P3 and the lifecycle bullets above are now command documentation.
