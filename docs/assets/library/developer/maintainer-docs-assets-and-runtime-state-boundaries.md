...
- Blocked by: W18 R10 Phase 5 and the W18 R7 and W18 R11 consumers of the store seam. Update when: the template runtime-state guidance refresh and the verification pass land, and when run-state storage and the reorganized command surface (tool-level `uninstall`/`update`, `setup remove`) start consuming `packages/cli/src/store/`. Guide change: revisit the runtime-state examples once the template guidance stops naming `.make-docs/runs/` as a current write target, and rewrite the seam-first lifecycle bullets as command documentation once the W18 R11 commands surface `pruneProjectFromStore` and `removeGlobalStore`.
