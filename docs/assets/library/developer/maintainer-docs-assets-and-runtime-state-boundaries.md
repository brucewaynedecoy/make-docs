...
- Store lifecycle behavior is seam-first because the command tree is owned by the W18 R11 reorganization: do not add new lifecycle commands or flags here. `pruneProjectFromStore` (`packages/cli/src/store/lifecycle.ts`) removes exactly one project's rows across all three tables in one transaction — it accepts a pre-resolved identifier because uninstall must capture identity before the manifest is removed, never creates a database, and never throws — and `removeGlobalStore` is the machine-footprint removal seam the W18 R11 tool-level `uninstall` will call: it unlinks only the fixed known store filenames non-recursively, removes the root directory only when empty, retains-and-reports anything unexpected, and refuses a root that carries project `.make-docs/` markers, so it is structurally incapable of deleting repository content.
- The store is never silently orphaned: the repo-level `make-docs uninstall` prunes the target project's rows and always prints the store disposition (kept for N other projects, last-project-gone with safe-removal guidance, no store, unresolvable identity, or an explicit prune warning). The repo-level command never removes the store — deleting a machine-level, all-projects store from a one-repo operation is the wrong blast radius — and the remove-or-prompt choice belongs to the W18 R11 tool-level `uninstall`. Store lifecycle handling runs strictly after the PRD 32-governed removal loop and preserves it byte-identically: `.make-docs/backup/**` destinations, legacy root `.backup/**` protection, and agentics pruning are unchanged, and `packages/cli/tests/store-lifecycle.test.ts` asserts both protected backup roots survive an uninstall with store handling active.
- Update migration and pre-v2 detection are verified at existing seams, not new code: every apply-shaped run migrates the store through `bootstrapGlobalStore`, and pre-v2 configurations hit the `classifyCompatibilityState`/`guardCompatibilityDisposition` warning-and-backup-or-cancel flow. The store's project-path records are local-only (R-PRIV-1): no store source may contain a network call or network-module import and the CLI's one network-capable module may never import the store — both directions are structurally enforced by `packages/cli/tests/store-lifecycle.test.ts` — and any future export or sharing surface requires explicit user opt-in before any store field leaves the machine.
...
- Blocked by: the W18 R7 and W18 R11 consumers of the store seam. Update when: W18 R7 run-state storage relocates the Playbook runner's `.make-docs/runs/playbooks/**` writes onto the `playbook_runs` seam, and when the reorganized W18 R11 command surface (tool-level `uninstall`/`update`, `setup remove`) surfaces the lifecycle seams. Guide change: fold the Playbook run-state relocation into the runtime-state story once nothing current writes under `.make-docs/runs/` (including retiring the tool-directory runtime-state classification caveat), and rewrite the seam-first lifecycle bullets as command documentation once the W18 R11 commands surface `pruneProjectFromStore` and `removeGlobalStore`.
