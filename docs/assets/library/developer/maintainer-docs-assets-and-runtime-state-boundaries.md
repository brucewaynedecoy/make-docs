...
- Blocked by: W18 R10 Phases 3 and 4 and the W18 R7 and W18 R11 consumers of the store seam. Update when: the project-state read/write operations with the checkpoint-evidence migration and the uninstall/`setup remove` lifecycle land — Phase 3 also wires the install-registry mirror upsert into the CLI apply flow, deliberately deferred from Phase 2 — and when run-state storage and the reorganized command surface start consuming `packages/cli/src/store/`. Guide change: extend the Machine-Level Global Store section with the evidence-migration boundary, the per-project pruning and machine-footprint removal flows, and refreshed maintainer checks for store-touching changes.
