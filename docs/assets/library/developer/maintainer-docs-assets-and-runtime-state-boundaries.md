...
- Blocked by: the W18 R7 store-seam consumer and the remaining W18 R11 tool-level commands. Update when: W18 R7 run-state storage relocates the Playbook runner's `.make-docs/runs/playbooks/**` writes onto the `playbook_runs` seam, and when the tool-level `uninstall`/`update` self-management commands land later in W18 R11 (`setup remove` already surfaces `pruneProjectFromStore`). Guide change: fold the Playbook run-state relocation into the runtime-state story once nothing current writes under `.make-docs/runs/` (including retiring the tool-directory runtime-state classification caveat), and rewrite the remaining seam-first lifecycle bullets as command documentation once the tool-level `uninstall` surfaces `removeGlobalStore`.
