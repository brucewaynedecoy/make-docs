...
One deliberate off-registry surface exists since W18 R13 Phase 2: the conformance kit generator (`packages/cli/src/conformance/kit.ts`, invoked through the maintainer-only `npm run conformance:kit` script) registers no operation and derives no CLI or MCP surface per [PRD 43](../../../prd/43-revise-conformance-scenario-model-and-execution-kit.md) R-HOME-1 — shipping it would advertise a maintainer-lab capability whose required assets are structurally excluded from every install. The parity rule is preserved vacuously, asserted in the standard suite by `listConformanceLabShippedSurfaceViolations`, which fails if any registry identifier or `run` CLI adapter names a conformance-lab surface; the revisit seam is recorded on register item [Q-022](../../../prd/03-open-questions-and-risk-register.md).
