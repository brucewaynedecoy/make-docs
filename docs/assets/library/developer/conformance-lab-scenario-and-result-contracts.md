...
related:
  ...
  - ../../../prd/44-revise-conformance-lab-execution-protocol-and-evidence-homes.md
...
A harness with no entry in `targets` is an uncovered target: `getScenarioTargetBinding` fails closed naming the gap, and the registry's scenario-absence notes report it — a reported gap, never implied coverage (R-SCHEMA-2, preserving PRD 37 R-SCEN-2's absence-must-be-reported rule structurally). This rule replaces the retired `futureHarnesses` list; Claude Code and Pi are currently uncovered targets on all four packaging definitions.

The schema is `.strict()` and fails closed on dishonest, stale, or superseded specs: the retired spellings — top-level `harness`, `harnessExecution`, `registryTupleIds`, `futureHarnesses`, and `characterization` — are rejected rather than silently ignored; the retired `--write` flag is rejected anywhere in a command step (scenario scripts use the PRD 41 `plan`/`preview`/`write`/`ship` grammar); a command step tagged `evidence-json` must literally pin `--json` so rendered TTY text never enters evidence (register item R-026); a `destructive` scenario must use the `destructive-temp-fixture-apply` safety mode; and `requiresNetwork`/`requiresCredentials` without a matching precondition kind is invalid.
...
The four required first-pass outcomes (PRD 37 R-SCEN-1 as revised by PRD 43 R-SCHEMA-3) are fixed in `REQUIRED_FIRST_PASS_SCENARIOS`, keyed by their domain-qualified ids — `packaging/plugin-marketplace-install`, `packaging/skills-bundle-discovery-invocation`, `packaging/dependency-check-both-directions`, `packaging/uninstall-backup-cleanliness` — and mapped to the user-visible outcome each must prove, with `REQUIRED_FIRST_PASS_TARGET` fixing Codex as the target every required definition must bind and `listMissingRequiredFirstPassScenarioIds` feeding the R-TEST-2 runnability check — absence of any definition is a failure, never a silent gap. All four are authored (`scenarioVersion` 2.1.0, harness-agnostic prose), `external-provider-run`, non-destructive, bar-complete, precondition-guarded, and bound to exactly one target today, Codex; none has run yet.

Every committed command step is executable as written against the current CLI — the three D-023 defect classes ([register item D-023](../../../prd/03-open-questions-and-risk-register.md)) are absent from the committed text, each fix attributed to the register item in the step's notes: ship steps carry `--support-evidence-ref conformance/tuple-registry.json#<tuple-id>` naming the exact registry tuple the session's evidence lands on (the planner unconditionally hard-stops without one); the non-TTY `setup backup` and `setup remove --backup` steps carry `--yes` so the TTY confirmation gate does not refuse them; and a workspace-setup step writes the packaging precondition attestations into the fixture workspace's `.make-docs/config.yaml` `packaging.preconditions` block before the ship step consumes them. Since W18 R13 Phase 2 the structural executable-by-construction guarantee exists as machinery: the kit generator projects every definition against the real command surface and drives the ship steps through the real pipeline under dry-run before any session starts (PRD 43 R-KIT-3; see The Execution Kit and Lab Sessions below), so the kit-generated projection — never the authored step text — is the executable projection. The generation dry run immediately earned its keep by catching two more latent defects the structural checks had passed: the probe fixture's `scenario-spec` reference prose parsed as a repository path (failing redistribution resolution), and all four definitions lacked the `make-docs setup --yes` workspace-establishment step their later steps assumed — both fixed in the committed definitions at `scenarioVersion` 2.1.0. The dedicated enforcing executability check in the meta-verification family, and D-023's register closure, remain the W18 R13 Phase 4 bar.
...
- `packaging/dependency-check-both-directions` binds its expectations to the v2 probe-based checks (PRD 40 R-DEP-3): its fixture set includes an `rg` dependency whose `source` prose begins with "ripgrep" (a check derived from prose would probe the wrong binary) and a deliberately absent probe target for the missing direction.
...
3. **Dry-run pipeline drive.** Every `run package ship` step is driven end to end through the operation core under the dry-run context — plan, preview, and write-with-no-writes via the shipped compiler and descriptors — against the freshly materialized workspace, so a plan stop that would abort a real session aborts generation instead.

The three D-023 defect classes are structurally impossible in generated output, asserted by tests over generated kits: missing support-evidence refs and missing `--yes` confirmation flags fail the static projection, and unestablished precondition attestations fail the dry-run pipeline because the kit's own workspace materialization is what supplies them. A failed generation removes any session artifacts it created before rethrowing, so no half-generated kit survives. No kit-local table of harness facts exists: every harness-specific string in kit output traces to a descriptor field — the interrogation block described in [Playbook Packaging and Harness Adapters](./playbooks-development-packaging-and-harness-adapters.md) — and a descriptor without the needed lab knowledge fails generation closed rather than inventing the fact (R-HOME-2; Pi currently has no interrogation block, so a Pi kit fails closed naming the gap).
...
| `uninstall` | A byte-level before/after diff of the placement roots and the workspace, proving managed outputs removed, user content untouched, and no orphaned managed directories. |
...
The three execution modes are documented as executable protocol content in [`conformance/operator-modes.md`](../../../../conformance/operator-modes.md) ([PRD 44](../../../prd/44-revise-conformance-lab-execution-protocol-and-evidence-homes.md) R-MODE-1..2) — this guide summarizes and links rather than duplicating. All three produce evidence through the same kit, instruments, and ingestion path, and each restates the R-EXEC-1 rule that only instrument outputs are evidence:

- **Human-only** — the manual fallback: one maintainer with a real harness install generates the kit, performs every step, runs each instrument by hand, and authors the attestations file. The parked `CONFORMANCE-RUN-codex-plugin.md` walkthrough (the user's untracked working file, absorbed as raw material and never modified by agents) is this mode's detailed reference; its boxed defect notes remain D-023's evidence record.
