...
- Blocked by: W18 R13 Phases 2-4 (PRD 43 R-KIT, R-INST, R-PROMPT, R-ING, R-HOME and [PRD 44](../../../prd/44-revise-conformance-lab-execution-protocol-and-evidence-homes.md)'s execution protocol). Update when: the per-target conformance kit generator (`packages/cli/src/conformance/kit.ts`), the deterministic bar-stage instruments, the ingestion step, the operator modes, and the lab-session evidence homes land. Guide change: document kit generation and its maintainer tooling entry point, the instrument outputs ingestion validates against the session manifest, the enforcing executability check as D-023's structural close, the descriptor's lab-facing interrogation block, and the lab-session vocabulary with the machine-level store's lab area as the retained-raw-evidence home.
