...
- Blocked by: W18 R13 Phases 3-4 (PRD 43 R-ING and the remaining verification bar; [PRD 44](../../../prd/44-revise-conformance-lab-execution-protocol-and-evidence-homes.md) R-MODE). The Phase 2 half of the former blocker — the kit generator, instruments, prompts, discovery kit, descriptor interrogation block, and lab-session evidence homes — is resolved and documented in The Execution Kit and Lab Sessions above. Update when: the deterministic ingestion step (instrument outputs to `conformance.result.v1` records through the unchanged recording seam), the `conformance/operator-modes.md` operator-mode instructions, and the enforcing meta-verification executability check land. Guide change: document ingestion's fail-closed assembly rules and attestation handling, summarize and link the three operator modes rather than duplicating them, and record the enforcing executability check as D-023's register close.
