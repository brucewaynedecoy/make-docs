...
Kit generation is maintainer lab tooling, deliberately NOT a shipped operation (R-HOME-1): it is invoked through `npm run conformance:kit` (`packages/cli/scripts/conformance-kit.ts`), registers nothing in the operation registry, and adds nothing to the CLI command tree or MCP surface — shipping it would advertise a maintainer-lab capability whose required assets the R-TEST-3 exclusion check structurally keeps out of every install, the D-022 category error repeated at the command level. The W18 R11 parity rule is preserved vacuously, asserted by `listConformanceLabShippedSurfaceViolations`, which fails the standard suite if any registry identifier or `run` CLI adapter ever names a conformance-lab surface; the revisit seam is recorded on register item [Q-022](../../../prd/03-open-questions-and-risk-register.md). Generation isolates the machine-level Store for its duration. The real session workspace binds setup to session-local Store state and keeps the resulting quiescence barrier active. The package preview proof supplies the proof workspace root only through its operation execution context or current working directory. A separate isolated Store-root binding supplies the proof-local Store and must never fall back to the operator Store. Neither binding changes the projected `package.ship` argument vector. Cleanup removes the proof-local Store on success or failure. No proof residue may remain outside intended session outputs, and the operator Store remains byte-for-byte unchanged. The intended session workspace and session-local Store are not proof residue.

### Executable by Construction

Every command a kit emits derives from the registered operation surface and the harness capability descriptors, and is proven against the real command surface before any session starts (R-KIT-3 — the structural close direction for register item [D-023](../../../prd/03-open-questions-and-risk-register.md)), in four layers:
...
3. **Proof isolation.** Generation creates the disposable proof-only workspace as a deterministic copy of the freshly materialized `workspace/`; hand construction is prohibited. The copy omits only `.make-docs/state/migration.lock.json`, `.make-docs/state/legacy-quiescence.json`, `.make-docs/state/legacy-writers/`, and `.make-docs/state/migration-receipts/`. A pre-execution byte-level comparison of relative paths and content must prove equality except for that exact allowlist. An unknown migration-coordination path or any other difference fails generation closed. Operation execution context or current working directory supplies only the proof workspace root. A separate isolated Store-root binding supplies the proof-local Store and must never fall back to the operator Store. Neither binding changes the projected `package.ship` argument vector. The proof workspace does not run setup. The real session workspace keeps its setup-created P5-through-P10 quiescence barrier. The proof workspace creates no barrier and never weakens the session barrier.
4. **Dry-run pipeline drive.** Every projected `run package ship` command and its exact argument vector are driven end to end through the operation core in the proof workspace — plan, preview, and write-with-no-writes via the shipped compiler and descriptors. Root binding must leave the vector byte-identical and in the same order. A plan stop or any result other than the accepted planned result aborts generation before session start. The proof is generation validation and is never presented as session execution evidence. A second byte-level comparison proves that `workspace/` stayed unchanged from proof setup through proof cleanup.

The three D-023 defect classes are structurally impossible in generated output, asserted by tests over generated kits: missing support-evidence refs and missing `--yes` confirmation flags fail the static projection, and unestablished precondition attestations fail the proof pipeline because the deterministic fixture copy supplies them without rerunning setup. Tests also pin the omission allowlist, fail-closed unknown paths and other differences, both byte-level comparisons, projected-argument equality before and after root binding, plan-stop failure, write-with-no-writes behavior, proof cleanup on success and failure, active session quiescence, no proof residue outside intended session outputs, an unchanged operator Store, and byte-identical kit output for equal inputs. Generation removes the proof workspace and proof-local Store on success or failure. A failed generation also removes any session artifacts it created before rethrowing, so no half-generated kit survives. No kit-local table of harness facts exists: every harness-specific string in kit output traces to a descriptor field — the interrogation block described in [Playbook Packaging and Harness Adapters](./playbooks-development-packaging-and-harness-adapters.md) — and a descriptor without the needed lab knowledge fails generation closed rather than inventing the fact (R-HOME-2; Pi currently has no interrogation block, so a Pi kit fails closed naming the gap).
