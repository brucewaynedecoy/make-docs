...
- `make-docs run playbook advance --repo-root <path> --run-id <id> --outcome completed|failed [--step <id>] [--evidence-ref <ref> ...] [--output-ref <ref> ...] [--note <text>]` records the cursor step's outcome and computes the next cursor.
...
- `make-docs run playbook resume --repo-root <path> --run-id <id> [--resume-hint <text> ...] [--evidence-ref <ref> ...] [--note <text>]` re-enters a held run at its stored cursor.
...
Since W18 R7 Phase 2, the five registry identifiers that W18 R11 registered as `pending` are active handlers: `playbook.next` (read) plus `playbook.advance`, `playbook.gate`, `playbook.resume`, and `playbook.close` (write) complete the PRD 35 R-OP-2 operation set alongside the earlier `start` and `status`. The engine is `packages/cli/src/operations/playbook/progression.ts`: pure position computation over (run state, parsed Playbook model, reported input), layered on the Phase 1 storage seam. Every mutation flows through `transitionPlaybookRunState`, each operation carries a zod input contract in `packages/cli/src/operations/playbook/ops/`, and each mutating operation registers `mutates: "write"` so registry dispatch applies the uniform operation-core safety gating (R-OP-1). The MCP tools derive automatically from the same registry entries, with CLI/MCP parity pinned by the derivation tests.
...
- Blocked by: the W18 R7 Phase 3 mode-execution and digest-aware-resume work and the Phase 4 guardrails, portability, and three-tier degradation work. Update when: Phase 3 lands the digest check at the marked `PHASE 3 SEAM (R-RESUME-1)` in `progression.ts` and step execution by mode, and Phase 4 lands the run-time guardrails. Guide change: replace the Progression Engine section's "resume does not digest-block" and "records reported outcomes" statements with the digest-blocked resume and per-mode execution semantics, add the guardrail and degradation behavior, and revisit whether the invoke-centric W18 R1 run description above should collapse into the progression flow.
