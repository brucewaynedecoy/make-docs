...
- Blocked by: the W18 R7 Phase 5 tests-and-verification pass (the D10 suite completion) and wave closeout. Update when: Phase 5 lands the completed per-operation, resume, mode, guardrail, and no-repo-run-state verification suite and the wave closes. Guide change: link the completed D10 verification coverage from the Progression Engine section, and revisit whether the invoke-centric W18 R1 run description above should collapse into the progression flow — the invoke-collapse question stays on this bullet rather than moving into Phase 5's scope, because Phase 5 is a verification phase and the collapse is a guide-maintenance decision best taken once the wave's verification confirms which invoke behaviors survive as a distinct surface. The Phase 4 half of the prior bullet is resolved: the run-time guardrails (R-GUARD-1..4), export/import portability (R-PORT-1), and the three-tier degradation proof (R-TIER-1) are documented in the Entry Points, Progression Engine, and Nested And Parallel Playbooks sections above, and the "R-TIER-1 groundwork" qualifier on the presented-command path is retired.
