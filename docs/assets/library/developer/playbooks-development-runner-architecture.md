...
Since W18 R12 Phase 3, the `run` dispatcher carries a render layer at the seam where `printJson(invocation.value)` previously sat ([PRD 41](../../../prd/41-revise-cli-human-experience-and-package-grammar.md) R-RENDER-1..3), implemented in `packages/cli/src/run/render.ts` and keyed by the previously unused `OperationRenderMode` in `packages/cli/src/operations/types.ts` — the seam PRD 41 notes was anticipated and never built. Mode selection is the whole contract: `--json` always emits the full operation result, a non-TTY stdout defaults to the same full JSON so pipes, scripts, and agents observe no change without passing any flag, and only a TTY defaults to `text`. The agent-invariance proof (R-INV-1, R-TEST-4) lives in `packages/cli/tests/run-cli-experience.test.ts`, which captures both channels and asserts them byte-identical (`toBe` string assertions, not deep-equal) to the pre-render-layer `printJson` output; because vitest's captured stdout is never a TTY, `runRunCommand` takes an injectable `isTty` seam that tests use to force text mode, while production reads `process.stdout.isTTY`.
