...
- Blocked by: W18 R6 Phase 3. Update when: the layered validator and completed diagnostic catalog join the single parsed Playbook model and staged parser that W18 R6 Phase 2 landed as the pure library at `packages/cli/src/playbook/` (entry point `parsePlaybook`), and together they replace the W18 R4 catalog validation described above, which remains what the code enforces until then. Guide change: replace the Catalog Contract Validation section's implemented-behavior description with the model/parser/validator architecture and keep the Playbook contract as the schema authority.
