...
A successful apply also sets up a small machine-level store at `~/.make-docs/` in your home directory: a global config, a global manifest, and a SQLite database for operational state such as run records. It never contains your project content, and store problems never block an install — if the store cannot be created, or your Node.js version is older than 22.5 so the database part is unavailable, the CLI prints a warning and completes the project install normally. The project's own `.make-docs/manifest.json` also records a `projectId`, a random identifier minted once on the first apply that keys this project's entries in that machine-level store; later syncs never change it, and it is never derived from your directory path, so it survives moving or cloning the project.
