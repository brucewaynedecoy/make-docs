...
A **preflight instrument** (`kit/instruments/preflight.mjs`) runs first, before any setup or bar-stage instrument: it compares `make-docs --version` to the manifest's `generationInputs.cliVersion` and refuses loudly (exit non-zero, with `just install-cli` reinstall guidance) on a mismatch or error, so a stale or mismatched global `make-docs` cannot silently corrupt a run (register item [D-027](../../prd/03-open-questions-and-risk-register.md)). The rendered `session-steps.sh` runs it as its first line, and ingestion refuses a session whose preflight recorded `ok:false`.
...
- [Playbook Packaging and Harness Adapters](../../../.make-docs/archive/legacy-playbooks/library/developer/playbooks-development-packaging-and-harness-adapters.md)
