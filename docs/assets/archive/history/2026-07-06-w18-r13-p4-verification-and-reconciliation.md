...
- **D-024** (repo-local transcript home) — **Closed**: no live surface names `.make-docs/conformance/` as a current home, the `.gitignore` entry is retired, the lab-session vocabulary and evidence homes are code, and the PRD 20/PRD 37 clauses are change-managed through PRD 44 R-NAME. The maintainer's local probe residue stays an operator to-do, not a code defect.
