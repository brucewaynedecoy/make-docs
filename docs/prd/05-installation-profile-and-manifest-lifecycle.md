...
- Enhanced by [32-revise-lifecycle-backup-state-agentics-pruning.md](./32-revise-lifecycle-backup-state-agentics-pruning.md) for lifecycle backup state and selected-agentics pruning. Future backup writes move from root `.backup/**` to `.make-docs/backup/**`, root `.backup/**` remains protected legacy backup state, and empty managed `.make-docs/agentics/**` directories are pruned only when audit proves no unmanaged descendants remain.
- Enhanced by W18 R4 for Run Playbook state separation. Playbook run state belongs under `.make-docs/runs/playbooks/**`; `.make-docs/manifest.json` remains managed installation ownership state and `.make-docs/config.yaml` remains project-owned configuration, including reviewed harness capability records.
...
- Backup consumes a single audit snapshot, maps project paths directly and home-scoped skill paths under `_home`, then copies only audited removable files and materialized empty directories to the reviewed backup destination in `packages/cli/src/backup.ts:86-158` and `packages/cli/src/manifest.ts:155-160`. PRD 32 revises the future destination from root `.backup/YYYY-MM-DD...` to `.make-docs/backup/YYYY-MM-DD...` while preserving legacy root `.backup/**` as protected state. The behavior is verified for project files, global skills, ordinals, cancellation, and noop runs in `packages/cli/tests/backup.test.ts:127-360`.
...
- Backup and uninstall both protect backup roots from recursive self-damage. PRD 32 requires audit and uninstall to protect both future `.make-docs/backup/**` state and legacy root `.backup/**` state. The legacy root exclusion is asserted in `packages/cli/tests/audit.test.ts:493-518` and `packages/cli/tests/lifecycle.test.ts:76-100`.
