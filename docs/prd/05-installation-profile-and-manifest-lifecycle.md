...
Installation records live in the global Store under [PRD 38](38-global-store-and-project-state.md). Project files show the selected content footprint; CLI status shows the applied installation record. Existing local manifests are transfer inputs only.
...
- Reconfiguration is the same planner/apply pipeline with a different selection source. `planInstall` in `packages/cli/src/install.ts` always resolves a fresh profile, while `applyInstallPlan` applies actions and commits a new Store installation snapshot. The round-trip from full install to partial profile and back is covered in `packages/cli/tests/install.test.ts`.
- Conflict handling is review-first for selected managed files. Divergent paths surface through file-scoped preserve-as-project-owned, export-then-replace, proven-managed overwrite, skip, or stop decisions before apply; unresolved reviewable diffs cannot be applied, and non-interactive runs fail instead of inferring ownership or disposition. Reviewed content exports may remain local. Conflict decisions, pending-replacement references, and recovery metadata live in the Store. Replacement file bodies remain in verified provider, backup, or approved content-copy locations. Selected desired diffs resolve before writes begin.
...
- Each configured harness receives an unconditional router foundation at the project root, `docs/`, `.make-docs/`, `.make-docs/system/`, and `.make-docs/system/{contracts,prompts,references,templates}/`. The resolved effective profile and its dependencies control capability-local routers at `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/`. An all-four effective profile has 12 unconditional or capability surfaces per harness, so two configured harnesses produce 24 router files before on-demand surfaces. On-demand `docs/assets/` has configured-harness root routers and no managed routers in legacy asset families or Persona subdirectories. `.make-docs/archive/` and `docs/assets/` receive configured-harness root routers only when created. Persona testing remains on demand and is routed from `docs/assets/`. Resource projection controls bodies only.
- The Store-owned installation manifest also records stable project identity, effective capability and skill selections, router ownership, selected resource projection, managed-file and managed-block snapshot identifiers, digests, bounded provenance, and content references, adoption receipts, last reviewed classification snapshot identity, and lifecycle disposition. A missing, stale, malformed, incomplete, ambiguous, or contradictory manifest cannot authorize destructive change.
...
- [22-project-documentation-asset-model.md](./22-project-documentation-asset-model.md) owns `.make-docs/archive/**`, `docs/assets/project/**`, and persona testing placement; PRD 21 owns `.make-docs/system/**`, while [47-persona-model.md](./47-persona-model.md) owns persona metadata. The manifest may record provenance for selected shipped resources, but never claims project-owned documentation merely from directory placement.
...
- General lifecycle `runs` and bounded `run_evidence` live in the Store. Installation ownership is also Store-authoritative, while project document meaning remains repository-authoritative. Opaque legacy `playbook_runs` stays untouched under [PRD 38](38-global-store-and-project-state.md).
- Setup preserves or mints declarative project identity in `.make-docs/config.yaml` and binds a separate Store-owned checkout id. Saved intent does not prove applied ownership. Project removal does not prune Store rows without a separate reviewed action.
...
- Managed install state is a Store-owned installation record. It includes schema and package identity, project and checkout ids, timestamps, profile id, raw and effective selections, resource projections, file and block ownership, provenance, hashes, routers, dispositions, and adoption receipts. The CLI must migrate supported `.make-docs/manifest.json` inputs safely and must never write a current local operational manifest.
...
### Reviewed Asset Layout Changes

R-ASSET-INSTALL-1 (MUST): fresh setup excludes the assets root from the unconditional foundation. Preserve existing content; when the root is needed, create only its configured-harness routers under PRD 15. Do not restore obsolete empty asset families on setup or repeat.
...
- Affected requirement or section: `Selection and Manifest Invariants`
- Previous contract: The router selection and manifest contract did not separate the unconditional foundation from capability-local documentation routers or state the root-only `docs/assets/` rule.
- Replacement contract: Every configured harness receives the unconditional foundation. The resolved effective profile and its dependencies control the four capability-local documentation routers. `docs/assets/` has one managed root router. Archive, artifact, and Persona testing surfaces remain on demand.
...
- Affected requirement or section: `Reviewed Asset Layout Changes` and current asset, bootstrap, migration, or storage statements in this owner.
- Previous contract: The install foundation included docs/assets/ and counted 13 surfaces per harness before on-demand work. Prior dated records retain their historical claims.
