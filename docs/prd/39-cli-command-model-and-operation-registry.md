...
- R-REG-6 (MUST): the `all` MCP profile is the exact deduplicated union of the intent profiles. A tool assigned to several profiles retains one stable name, schema, description, access fact, handler, and operation ID. Filtering a profile does not modify registry admission or the shared operation core. `defaultExposure` metadata may advise a host to defer loading but is not authorization or a protocol promise.
