...
- R-REG-5 (MUST): every MCP-ready operation definition declares explicit `mcp.profiles` with one or more known intent profiles. The derived MCP descriptor carries that assignment. Hand-defined MCP tool descriptors carry the same metadata. Profile membership is not inferred from an operation name, tool name, URI, or access class. A newly ready operation cannot enter the MCP catalog without a profile assignment.
- R-REG-6 (MUST): the `all` MCP profile is the exact deduplicated union of the intent profiles. A registry-derived tool assigned to several profiles retains one stable operation ID, name, schema, description, access fact, handler, and write policy. A hand-defined tool assigned to several profiles retains one stable name, schema, description, access fact, handler, and write policy. Filtering a profile does not modify registry admission or the shared operation core. `defaultExposure` metadata may advise a host to defer loading but is not authorization or a protocol promise.
