...
- Enhanced by [22-revise-new-docs-assets-playbooks-persona-model.md](./22-revise-new-docs-assets-playbooks-persona-model.md) for package proof of managed project documentation assets. Packed validation must cover template copy, dogfood parity, frontmatter/persona fixtures, and path-hygiene checks for `docs/assets/{archive,artifacts,breadcrumbs,guides,playbooks}/` while proving top-level `docs/archive/**` and top-level `docs/artifacts/**` are not shipped v2 targets.
