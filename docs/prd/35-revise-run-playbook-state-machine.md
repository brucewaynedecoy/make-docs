...
- Superseded by [41-revise-cli-human-experience-and-package-grammar.md](41-revise-cli-human-experience-and-package-grammar.md). The R-STATE-1 resume hints gain subject scoping and retirement semantics: each hint records the step or gate it advises about, every mutating transition retires hints whose subject has reached a resolved status, and `close` retires all guidance hints, so a closed run's state carries none. Hints are current guidance only; the evidence log remains the unchanged durable audit trail, and any serialization change is additive per the global store's schema-versioning rules.
