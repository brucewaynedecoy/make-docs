...
- R-KIT-3 (MUST): kit generation derives every emitted command and resource operation from the current registered public surface and fails before session start when a definition cannot project to an accepted command or operation sequence. It binds each projected `package.ship` command and its exact argument vector to a disposable proof-only workspace that is separate from the real session workspace. Generation must create the proof workspace as a deterministic copy of the freshly materialized session fixture; hand construction is prohibited. The only permitted omissions are `.make-docs/state/migration.lock.json`, `.make-docs/state/legacy-quiescence.json`, `.make-docs/state/legacy-writers/`, and `.make-docs/state/migration-receipts/`. Before proof execution, a byte-level relative-path and content comparison must prove equality except for this exact allowlist; an unknown migration-coordination path or any other difference fails generation closed. The proof root is bound only through the operation execution context or current working directory, and the projected argument vector stays byte-identical and in the same order. The proof uses a proof-local Store root, does not run setup, and does not create or weaken migration quiescence. It drives the exact projection through the real `package.ship` plan, preview, and write-with-no-writes path. A plan stop or any result other than the accepted planned result fails generation before session start. The proof is not session execution evidence. A byte-level comparison must also prove that the real session fixture remains unchanged from proof setup through proof cleanup. Generation removes the proof workspace and proof-local Store on success or failure. No proof residue may remain outside intended session outputs, and the operator Store must remain unchanged. The intended session workspace and session-local Store are not proof residue. The real session workspace runs setup normally and keeps P5-through-P10 quiescence active.
...
- R-TEST-2 (MUST): every current scenario referenced by the tuple registry exists and passes a kit-generation dry run against the current registered command/resource surface. Tests prove deterministic copying, the exact four-path omission allowlist, fail-closed unknown paths and differences, byte-level proof-copy equality, byte-level session-fixture stability through cleanup, byte-identical projected arguments before and after root binding, plan-stop failure, write-with-no-writes behavior, proof cleanup on success and failure, active quiescence in the real session workspace, no proof residue outside intended session outputs, an unchanged operator Store, and byte-identical kit output for equal inputs. Missing operations or preconditions produce `blocked`, never omission or a silent pass.
