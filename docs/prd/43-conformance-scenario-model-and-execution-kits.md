...
- R-KIT-3 (MUST): kit generation derives every emitted command and resource operation from the current registered public surface and fails before session start when a definition cannot project to an accepted command or operation sequence.
...
- R-TEST-2 (MUST): every current scenario referenced by the tuple registry exists and passes a kit-generation dry run against the current registered command/resource surface. Missing operations or preconditions produce `blocked`, never omission or a silent pass.
