...
| Backlog review cache: checkout identity, record path, deterministic record digest, snapshot schema version, rule catalog version, Skill version, bounded review fragment, and cache times | Store-cached and rebuildable | Backlog review cache service under PRD 51 through the shared Store service boundary | Explicit backlog-review cache readers, validation, pruning, and Store status | May be deleted, pruned, or rebuilt at any time; an invalid or unreadable row is a miss | Keep local; store repository-relative paths only; store no document bodies, prompts, secrets, raw logs, or absolute paths | Reuse an exact per-record review match only; never supply repository authority, a current snapshot, a portfolio conclusion, ownership proof, or required recovery state |
...
- Affected requirement or section: `Minimal State Classes and Durable Field Register`.
- Previous contract: The durable field register did not contain backlog-review cache records.
- Replacement contract: The Global Store can hold one privacy-bounded, exact-key, rebuildable per-record backlog-review cache. It is optional and non-authoritative. It contains no repository bodies, prompts, secrets, raw logs, or absolute paths. Its absence or failure cannot block a full stateless review.
- Rationale: Repeat reports can avoid repeated agent review for unchanged records while the repository and the current deterministic snapshot remain authoritative.
- Source: [PRD 51](51-backlog-review-and-reporting.md), [W23 R0 plan](../plans/2026-09-18-w23-r0-backlog-review-and-reporting/00-overview.md), and [P5 work record](../work/2026-09-18-w23-r0-backlog-review-and-reporting/05-incremental-review-cache-and-data-access.md).
