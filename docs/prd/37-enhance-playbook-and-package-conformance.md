...
- Superseded by [43-revise-conformance-scenario-model-and-execution-kit.md](43-revise-conformance-scenario-model-and-execution-kit.md), scenario identity and absence reporting only (W18 R13, register items D-025 and R-028). The four required first-pass outcomes are unchanged in substance, but they are no longer harness-named specs: definitions are harness-agnostic, organize by domain under `conformance/scenarios/<domain>/` with domain-qualified ids (`packaging/plugin-marketplace-install`, `packaging/skills-bundle-discovery-invocation`, `packaging/dependency-check-both-directions`, `packaging/uninstall-backup-cleanliness`), and bind execution targets through a per-definition `targets` map — Codex bound for the first pass, exactly as R-SCEN-1 requires. R-SCEN-2's rule that absent coverage is reported rather than implied survives re-expressed structurally: a harness with no `targets` entry is an uncovered target, kit generation for it fails closed naming the gap, and the registry's scenario-absence notes continue to report it — the `futureHarnesses` property is retired.
...
- Enhanced by [43-revise-conformance-scenario-model-and-execution-kit.md](43-revise-conformance-scenario-model-and-execution-kit.md) (W18 R13, register items D-023 and R-028). The R-TEST-2 check follows the domain-qualified scenario ids and `scenarios/<domain>/` paths, and the meta-verification family gains the executability check that D-023 proved missing: a kit-generation dry-run projects every required first-pass definition to a command sequence the current CLI accepts, so "runnable" is verified against the real command surface rather than structurally. R-TEST-1 and R-TEST-3 are unchanged (the R-TEST-3 markers detect the family's subtree fragments at any depth, which is verified — not assumed — to survive the `scenarios/<domain>/` nesting).
...
- Superseded by [43-revise-conformance-scenario-model-and-execution-kit.md](43-revise-conformance-scenario-model-and-execution-kit.md) and [44-revise-conformance-lab-execution-protocol-and-evidence-homes.md](44-revise-conformance-lab-execution-protocol-and-evidence-homes.md) (W18 R13, register items D-023/D-024/D-025 and R-028), organization and raw-evidence default only: scenario definitions live harness-agnostic at `conformance/scenarios/<domain>/` with domain-qualified ids and per-target `targets` bindings, committed result records live at `conformance/results/<harness>/`, and the executable projection of a definition is a generated per-target kit rather than hand-transcribed spec steps (PRD 43). The raw-transcript default below naming `.make-docs/conformance/` is rejected: transcripts and evidence scratch live in the disposable lab-session workspace, promoted evidence lands in committed result records, and retained raw evidence goes to the machine-level store's lab area (PRD 44 R-NAME-2). The tuple registry, its statuses and derivation, the result contract, the recording seam, and the maintainer-only boundary in this section are unchanged.
