...
- Superseded by [44-revise-conformance-lab-execution-protocol-and-evidence-homes.md](./44-revise-conformance-lab-execution-protocol-and-evidence-homes.md) (W18 R13, register items D-024 and R-028), raw-evidence default and execution protocol only: the lab-scope default below that raw run artifacts, transcripts, provider logs, and temporary workspaces go to `.make-docs/conformance/` or `.make-docs/runs/conformance/` is rejected — evidence scratch lives in the disposable lab-session workspace and is discarded with it by default, deliberately redacted-and-promoted evidence lands in committed result records, and raw evidence retained beyond a session goes to the machine-level store's lab area, never repo-local `.make-docs/`. The lab's execution protocol is revised in the same doc: agent-driven self-assessment becomes the primary execution mode under the rule "the agent drives, the instruments measure" (evidence comes exclusively from deterministic instrument outputs, never from a target agent's claims), three operator modes are first-class, and the operational envelope is named a "lab session" so "run" survives only as the recorded-run evidence noun and the `run` CLI command. The lab's maintainer-only scope, scenario contract, safety modes, result contract, verdicts, evidence classes, support-claim gating, and one-run threshold are unchanged.
