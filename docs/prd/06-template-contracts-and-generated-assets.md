...
- The shipped surface is the consumer tree described in `README.md` and `packages/docs/README.md`: the installed provider exposes product-owned `contract`, `prompt`, `reference`, and `template` bodies; configured-harness routers are always installed at the project root, `docs/`, `docs/assets/`, `.make-docs/`, `.make-docs/system/`, and all four typed system directories; the resolved effective profile and its dependencies control capability-local routers at `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/`; `docs/assets/` has one managed router at its root and none below it; an explicitly selected projection may add resource bodies under `.make-docs/system/{contracts,prompts,references,templates}/`; Make Docs-managed archive/provenance records use on-demand `.make-docs/archive/**`; non-authoritative source and analysis inputs use on-demand `docs/artifacts/**`; persona-scoped reader assets and testing evidence use on-demand `docs/assets/<persona-slug>/**` and are routed from `docs/assets/`; and mutable CLI state stays at `.make-docs/manifest.json` plus `.make-docs/conflicts/<run-id>/`.
...
- Configured project-root, documentation-root, documentation-capability, `.make-docs/`, `.make-docs/system/`, typed-system-directory, and on-demand archive/artifact `AGENTS.md` or `CLAUDE.md` routers are template-sourced managed-block files for supported harnesses. `docs/assets/` is part of the unconditional foundation and its router is root-only. Routers at `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/` are capability-local and exist only for document types enabled by the resolved effective profile and its dependencies. [15-agent-instruction-ownership-and-managed-blocks.md](./15-agent-instruction-ownership-and-managed-blocks.md) owns their block-scoped behavior.
...
- Template-owned files include the unconditional project-root, `docs/`, `docs/assets/`, `.make-docs/`, `.make-docs/system/`, and typed-system-directory instruction routers; profile-controlled capability-local `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/` routers; on-demand archive/artifact instruction routers; shipped workflow references; shipped templates; shipped prompt starters; starter documentation structure; and static helper files recorded by the installer as managed assets.
...
- On-demand `.make-docs/archive/`, `docs/artifacts/`, and `docs/assets/<persona-slug>/` paths are directory-contract surfaces. Archive and artifact routers may be template-owned. Persona subdirectories have no managed routers and use the root-only `docs/assets/` router. Local records remain project-owned or explicitly managed according to the owning contract. `.make-docs/` runtime state and archive/provenance records are never reseeded through `docs/`.
...
- Harness and directory router expansion happens in `packages/cli/src/types.ts` and `packages/cli/src/catalog.ts`. Enabled harnesses become active instruction kinds (`AGENTS.md`, `CLAUDE.md`). Each supported instruction kind receives the unconditional foundation at the project root, `docs/`, `docs/assets/`, `.make-docs/`, `.make-docs/system/`, and all four typed system directories. The resolved effective profile and its dependencies add only the matching `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/` routers. An all-four effective profile has 13 surfaces per harness, so two supported harnesses produce 26 router files. First-created archive or artifact surfaces receive on-demand routers. Persona subdirectories do not receive routers. Unsupported router shapes are reported rather than invented.
...
- Project-root, `docs/`, `docs/assets/`, `.make-docs/`, `.make-docs/system/`, and typed-system-directory instruction routers remain unconditional bootstrap surfaces. `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/` routers are capability-local and follow the resolved effective profile and its dependencies. The installed provider supplies resource bodies without local snapshots, and any selected body projection must retain trustworthy provenance rather than silently replacing package bytes; [17-system-asset-materialization-and-local-bootstrap.md](./17-system-asset-materialization-and-local-bootstrap.md) owns the detailed contract.
