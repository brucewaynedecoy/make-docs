...
- Superseded by [15-revise-agent-instruction-file-ownership.md](./15-revise-agent-instruction-file-ownership.md) for root instruction rendering. Root `AGENTS.md` and `CLAUDE.md` are now rendered as mirrored delimited managed blocks with inline routing; no dedicated `.make-docs/` instruction files are shipped.
...
- Superseded by [15-revise-agent-instruction-file-ownership.md](./15-revise-agent-instruction-file-ownership.md) for generated instruction assets. The template package now includes inline root instruction blocks as managed assets, and root instruction files are reconciled by managed block rather than whole-file overwrite/skip semantics.
