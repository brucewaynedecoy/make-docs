...
| Status | Decision | Follow-Up |
| --- | --- | --- |
| Open | Two correlated gaps confirmed on 2026-07-04, both from the same pre-W18 implementing-agent era that produced the playbook drifts. First, the `custom/` directories under `.make-docs/contracts/`, `.make-docs/references/`, and `.make-docs/templates/` — each with router and agent-instruction files per configured harness — mandated by [21-revise-tool-directory-system-custom-resource-tiers.md](./21-revise-tool-directory-system-custom-resource-tiers.md) are never created by setup; the installed instance carries only the `system/` tiers. Second, the no-scripts migration lineage [26-revise-no-scripts-migration-skill-refactor.md](./26-revise-no-scripts-migration-skill-refactor.md) (W16 R3) intended operational scripts to be built into the CLI, yet `check_path_hygiene.py` still ships as a repo-local Python script at `.make-docs/scripts/check_path_hygiene.py` and the intended `.make-docs/scripts/` thin discovery shims — thin wrappers plus router files making CLI operations discoverable to agents — were never implemented. An open sub-question qualifies the second gap: the W18 R11 operation registry plus MCP ([39-revise-cli-command-reorganization-and-operation-registry.md](./39-revise-cli-command-reorganization-and-operation-registry.md)) may now supersede shell shims as the agent discovery mechanism, making the right fix router-doc updates pointing at the registry-derived surfaces rather than shims. The custom-directories gap is additionally superseded in direction by the Q-017 instance-layout centralization lineage, which questions whether per-project replication of system tiers should exist at all. | Run a bounded PRD-conformance audit across the affected pre-W18 lineages (PRD 21, PRD 26, and adjacent same-era PRDs) to enumerate remaining drift in one sweep rather than serial discovery; resolve the shims-versus-registry sub-question before implementing either fix; and sequence the custom-tier fix behind the Q-017 direction decision so setup does not start creating directories the centralization design would remove. |
...
**Recommendation**: Audit the pre-W18 PRD set for implementation conformance in one bounded pass, decide whether the W18 R11 registry and MCP supersede shell shims (likely making router-doc updates the fix), and treat the custom-tier gap as an input to the Q-017 centralization design rather than implementing it as specified first.

**To close**: The bounded conformance audit is recorded with each enumerated gap dispositioned, the shims-versus-registry discovery decision is explicit, and the custom-tier gap is either implemented per PRD 21 or superseded by the accepted Q-017 design.
...
**Recommendation**: Hold this as a named future design lineage rather than an increment on setup; any interim work on the D-019 custom-tier gap or the Q-018 configuration IA should treat this direction as a known successor and avoid entrenching per-instance replication further.
...
| Status | Decision | Follow-Up |
| --- | --- | --- |
| Open | Captured from the 2026-07-04 second UAT as the design lineage that promotes and expands D-017's scope beyond schema documentation. Today's configuration surfaces are fragmented across `.make-docs/config.yaml` (YAML, partly hand-authored), `.make-docs/manifest.json` (tool-owned JSON), the global store's `config.json`, `manifest.json`, and `store.db`, and hand-inserted config blocks whose value vocabularies — for example the tri-state packaging precondition `satisfied | unknown | unsupported` — are discoverable only from source code. The design should answer format, ownership (hand-editable versus tool-owned), discovery path (progressive-disclosure and self-describing, like the instruction-file routers), and self-documentation (for example a config surface that explains its keys and vocabularies to users and agents). D-017 is updated in place to point here as its resolution path rather than at a standalone reference doc. The lineage interacts with the Q-017 instance-layout centralization question — the two should be designed together or in explicit sequence — and consumes the presentation-only boundary from [24-revise-configuration-convention-overlay.md](./24-revise-configuration-convention-overlay.md) and the store-owned surfaces from [38-revise-global-store-and-project-state.md](./38-revise-global-store-and-project-state.md). | Start the configuration information architecture design lineage covering format, ownership, discovery, and self-documentation across the project-local and store-backed config surfaces, deliver the D-017 config-schema reference through it, and sequence it with or explicitly against Q-017. |

**Question**: What is the coherent information architecture for configuration — which formats, which files, which owner (hand versus tool), how users and agents discover what is configurable, and how the config surface documents its own keys and vocabularies?
...
**Recommendation**: Design the configuration IA as one lineage answering format, ownership, discovery, and self-documentation together, rather than shipping the D-017 reference as a standalone artifact against today's fragmented layout.

**To close**: An accepted configuration IA design exists that assigns format, ownership, discovery path, and self-documentation for every configuration surface, and the D-017 close bar is delivered through it.

### Q-019 How Should the Persona Model Revise and Gain Interactive Setup?

| Status | Decision | Follow-Up |
| --- | --- | --- |
| Open | Re-captured from the 2026-07-04 second UAT as previously stated direction on the W9 R3 persona lineage (Q-009, closed by [22-revise-new-docs-assets-playbooks-persona-model.md](./22-revise-new-docs-assets-playbooks-persona-model.md); downstream consumers tracked by R-011): the CLI needs an interactive personas setup surface, and the persona primitives should move from three to four — `agent`, `maintainer`, `user`, `system` — with the `developer` naming replaced by `maintainer`. All custom personas inherit from a primitive (for example `claude-code` and `codex` from `agent`; `developer` and `architect` from `maintainer`). Personas are stored in the project-local `.make-docs/` and plausibly also the global store, which would let playbooks and runs bind to projects and personas. The storage decision explicitly depends on the Q-018 configuration information architecture design. | Once the Q-018 storage question is answered, raise a PRD change candidate on the PRD 22 persona lineage for the four-primitive schema, the maintainer rename, and persona storage, and design the interactive personas setup surface against it. |
...
| Status | Decision | Follow-Up |
| --- | --- | --- |
| Deferred | Captured from the 2026-07-04 second UAT and explicitly flagged by the user as half-baked: the W18 R12 dependencies table-to-YAML conversion made the document more readable, prompting the question whether other currently-prose Playbook sections would benefit from imposed structure. The evaluation frame is recorded with it: the litmus is whether code parses a section's content — machine-meaningful data belongs in structured blocks, while genuinely narrative sections such as Purpose and Step Guidance serve the tier-1 human-followability guarantee as prose — and dependencies was the only section where machine data lived in prose. | Revisit with authoring evidence after W18 R9 and further UAT rather than acting now; apply the does-code-parse-it litmus to any candidate section, and do not structure narrative sections that no code consumes. |

**Question**: Should any further Playbook section move from prose to imposed structure, and by what test?

**Why it matters**: The dependencies conversion showed structure can improve rather than harm readability when the content is machine-meaningful, but structuring narrative sections would erode the tier-1 hand-followable guarantee that makes a Playbook useful without any engine.

**Recommendation**: Keep the litmus — structure only what code parses — and gather authoring evidence from W18 R9 and subsequent UAT before proposing any contract change on the PRD 40 lineage.

**To close**: Authoring evidence either motivates a scoped PRD 40 lineage change for specific sections or this direction is explicitly retired with the litmus recorded in the contract lineage.
