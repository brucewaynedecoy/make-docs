...
| Status | Decision | Follow-Up |
| --- | --- | --- |
| Open | Skill refactor is now scoped by [26-revise-no-scripts-migration-skill-refactor.md](./26-revise-no-scripts-migration-skill-refactor.md): deterministic logic must move into CLI/shared-core operations first, and `closeout-commit`, `closeout-phase`, `work-on-wave`, and `work-on-phase` skills must be rewritten in the same implementation window before standalone helper scripts are removed or downgraded. W16 R3 implements the lifecycle-critical operation boundary, rewrites those four skills to call `make-docs operations`, removes the replaced lifecycle helper scripts from selected-skill registry assets, and adds selected-skill install/audit/backup/uninstall/smoke-pack validation. The risk remains open for remaining helper scripts, purpose metadata, future MCP/Rust parity, and broader skill delivery decisions. | Keep the W16 R3 evidence as the lifecycle-helper mitigation; close this only after remaining first-party skill helper dependencies, package/template validation, purpose metadata, and delivery-model work prove selected skills no longer rely on standalone scripts for make-docs-owned deterministic behavior. |
...
**Recommendation**: Treat W16 R3 as the lifecycle-helper mitigation baseline and require later skill, purpose-manifest, MCP, Rust, and shared-agentics work to reuse the packaged CLI/shared-core boundary.
...
**Why it matters**: W16 R3 covered closeout/work lifecycle helpers, but later script, plugin, Rust, MCP, and shared-agentics moves can recreate the same sequencing failure if they are not operation-first.
