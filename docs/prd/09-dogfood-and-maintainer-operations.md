...
- Enhanced by [22-revise-new-docs-assets-playbooks-persona-model.md](./22-revise-new-docs-assets-playbooks-persona-model.md) for dogfooding managed project documentation assets. Future archive, artifact, breadcrumb, guide, and playbook defaults must be reseeded and parity-checked through `docs/assets/{archive,artifacts,breadcrumbs,guides,playbooks}/`, while `docs/library/playbooks/**`, former top-level `docs/artifacts/**`, top-level `docs/archive/**`, and existing `docs/assets/history/**` remain transitional migration-lab evidence until planned relocation or preservation.
