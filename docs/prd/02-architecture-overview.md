...
3. Desired-state construction: `packages/cli/src/profile.ts:68-93` computes `capabilityState`, `effectiveCapabilities`, and `profileId`; `packages/cli/src/rules.ts:120-194` and `packages/cli/src/catalog.ts:64-85` build the desired docs asset set; and `packages/cli/src/skill-catalog.ts` plus `packages/cli/src/skill-resolver.ts:40-244` build the desired shared payload and native harness exposure asset set when skills are enabled.
