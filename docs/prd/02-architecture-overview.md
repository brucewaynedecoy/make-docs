...
The consumer-install zone is the target repository plus optional home-directory skill scope. Its unconditional bootstrap consists of configured-harness routers at the project root, `docs/`, `.make-docs/`, `.make-docs/system/`, and `.make-docs/system/{contracts,prompts,references,templates}/`, plus declarative project identity/config. The resolved effective profile and its dependencies add capability-local routers at `docs/designs/`, `docs/plans/`, `docs/prd/`, and `docs/work/` only for enabled document types. The on-demand `docs/assets/` surface has configured-harness root routers only. Optional managed surfaces include system-resource bodies, `.make-docs/archive/**`, `docs/assets/project/**`, Persona asset or testing children beneath `docs/assets/`, and approved local backup or content exports when first needed. Explicitly selected Skills install at the standard scope/harness paths in [PRD 28](28-shared-agentics-installation-and-harness-exposure.md): project Claude-only `.claude/skills/<name>`, project Codex-only or both `.agents/skills/<name>`, and global `~/.agents/skills/<name>` with selected additional native links or supported copies. Project single-harness selection creates only its selected Skill root; the canonical native path is direct, never self-linked.
...
- Planning, apply, and manifest state: `createInstallPlan` and `createSkillsOnlyInstallPlan` in `packages/cli/src/planner.ts` compute `PlannedAction[]`; `applyInstallPlan`, `applySkillsOnlyInstallPlan`, and their shared internal apply path in `packages/cli/src/install.ts` apply actions, commit Store installation state, and record conflict decisions and pending operations in the Store.
...
1. Command ingestion: `packages/cli/src/index.ts` invokes `runCli`; `parseArgs` and `validateParsedArgs` in `packages/cli/src/cli.ts` parse and validate argv; the Store service resolves the checkout installation record; and `runCli` decides whether the run is apply, reconfigure, skills, backup, or uninstall.
...
The Store owns applied installation state, including schema and package identity, profile, effective selections, files, Skill ownership, hashes, and recovery progress. Project-owned `.make-docs/config.yaml` holds declarative project identity and desired settings. `.make-docs/manifest.json` is legacy transfer input only.

PRD 24 defines optional `.make-docs/config.yaml` as project-owned convention configuration. That file is separate from applied installation state. Its presentation fields influence rendering only; canonical path selection, metadata validation, package copy, provider/cache behavior, and agentic routing still use canonical identifiers.
...
### Project Asset Discovery Boundary

R-ASSET-ARCH-1 (MUST): shared material uses `docs/assets/project/**`; audience material uses `docs/assets/<persona-slug>/**`. The reserved `project` segment is not a Persona. Resolve the two fixed `user` and `maintainer` defaults plus configured custom entries under PRDs 24 and 47. A human or agent can fill either audience role.
...
- Affected requirement or section: `Project Asset Discovery Boundary` and current asset, bootstrap, migration, or storage statements in this owner.
- Previous contract: The consumer bootstrap included an unconditional assets root and sent shared inputs to docs/artifacts/. Prior dated records retain their historical claims.
