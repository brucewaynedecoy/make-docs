...
The skill configuration surface is split between packaged metadata and remote content sources. `packages/cli/skill-registry.json:3-112` defines which skills exist, which source payloads are resolved, and which extra assets belong in the canonical shared payload; `packages/cli/src/skill-registry.ts:25-114` validates that metadata; `packages/cli/src/skill-resolver.ts:118-244` normalizes and fetches the remote payloads; and `packages/cli/src/skill-catalog.ts` generates the harness stubs from the resolved selection metadata.
