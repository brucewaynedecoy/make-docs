...
This artifact is a recommendation for how to begin v2 planning. It is not a design, plan, PRD change, work backlog, guide, or history record. It should be treated the same way as the other files in `docs/artifacts/`: useful input for downstream lifecycle work, not authority by itself.
...
7. **New Docs Assets, Playbooks, and Persona Model**
   - Move `docs/guides/` into `docs/assets/guides/` after the current template-owned `docs/assets/**` content has a migration path into the tool directory.
