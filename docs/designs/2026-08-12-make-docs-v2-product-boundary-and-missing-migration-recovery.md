...
The safety lattice is monotonic: any `unknown`, `mixed`, `managed-modified`, `incomplete`, `ambiguous`, or `contradictory` provenance, malformed provenance, newer-unknown Store, or contradictory ownership prevents an unattended migration for that facet. A lower-confidence facet cannot be overridden by a higher-confidence unrelated facet.
