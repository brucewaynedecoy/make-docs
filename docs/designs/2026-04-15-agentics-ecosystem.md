...
This design captures the conceptual model for evolving `make-docs` from a single-template installer into a multi-capability ecosystem. The ecosystem ships skills, hooks, prompts, agent definitions, workflow configs, reference docs, templates, and docs-repo wiring — all as optional, independently installable units — reached through a single gateway skill and a capability registry. The goal is a coherent way to grow the catalog additively without overwhelming consumers or bloating the CLI.

## Context

`make-docs` v1 ships one artifact: a `docs/` template. v2 needs to ship many artifacts, each optional, each with a different audience. No consumer will want the full set; every consumer will want a different subset. Without an organizing model, the repo drifts toward a kitchen-sink installer and the CLI toward an ever-growing switch statement.
...
| **Role** | A consumer archetype — "solo dev", "product team", "docs repo admin", "maintainer of a make-docs-adopting project". Roles map to recommended module sets so first-time installs do not require picking from every module. |
...
A single skill — `make-docs` (alias `make-docs-gateway`) — is the user-facing entry point. Its job is not to do the work; its job is to route. The gateway:
...
- `make-docs` (no args) — install the baseline template plus the gateway skill.
- `make-docs add <module>` — add a module. Resolves via the registry.
- `make-docs remove <module>` — remove cleanly, reversing all installation effects.
- `make-docs list` — enumerate installed and available modules.
- `make-docs doctor` — run healthcheck across installed modules; the gateway skill can invoke this as well.
- `make-docs role <role>` — interactive wizard keyed off a role ("solo dev", etc.) that suggests a bundled module set.

The `cli-bridge` skill lets an agent drive the CLI invisibly — a user tells an agent "install the PRD module" and the agent runs `make-docs add prd-module`, cleans up, and confirms.

### Wiring Flavors

Consumers who version-control `docs/` in a separate repo install one of four flavors via `make-docs add wiring:<flavor>`:
...
- `post-prd-generate` — runs `make-docs doctor` after PRD files are written.
...
Workflows are YAML configs that chain skills into repeatable multi-step tasks with self-reflection and optional review gates. The execution harness is a separate project; `make-docs` ships only the configs. Sketch shape:
...
`make-docs` guarantees only the config shape and the reference mapping into its registry. The gateway skill can optionally invoke workflows when a harness is detected.
...
- **Gateway auto-load vs `/make-docs` invocation.** Auto-loading the gateway on every Claude Code session maximizes discoverability but is intrusive and noisy. Requiring `/make-docs` invocation is quiet but missable. Invocation-based is chosen; the CLI surfaces the `/make-docs` entry point during install so users learn it.
...
- **Default-install gateway vs opt-in.** If the gateway is opt-in, a fresh install has no entry point and the ecosystem is invisible. Default-installing the gateway during `make-docs` baseline means every consumer has one command from day one; the cost is one additional file in the consumer project.
