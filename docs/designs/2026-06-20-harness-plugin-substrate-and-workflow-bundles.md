...
The [v2 roadmap](../assets/project/v2-proposed-design-and-roadmap.md) places this design second in "Batch 4 - Playbooks and Plugins." It follows the accepted [Playbook Contract and Run Playbook](2026-06-20-playbook-contract-and-run-playbook.md) design and the Batch 3 shared agentics decisions. The roadmap asks this design to define plugin packaging, install, update, and uninstall behavior for supported harnesses; separate substrate from productized bundles; and make non-maintainer guardrails explicit.

This design intentionally starts from artifact roadmap inputs instead of from an already accepted plan or PRD. That is a lifecycle departure under the [documentation lifecycle](../../.make-docs/system/references/lifecycle.md): v2 planning is using artifact proposals as a source-to-design straddle, then returning to the default design -> plan -> PRD -> work -> implementation sequence for downstream work.
