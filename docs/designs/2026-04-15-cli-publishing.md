...
Capture the decision to publish the `make-docs` CLI to the public npm registry and the approach for going from the verified Phase 7 tarball to a real release. Until this design lands as executed work, `npx make-docs` does not resolve for external consumers; this document settles the path from a dry-run-verified artifact to a shipped package without surprises.

## Context

The `make-docs` CLI has never been published. Absent a published package, `npx make-docs` does not work and the tool is usable only from inside this repository. Publishing is the last missing piece that lets consumers in other projects pick the tool up.
...
- Availability of the package name `make-docs` on the public registry, verified with `npm view make-docs`. If a package exists under that name, the release switches to a scoped name such as `@<user>/make-docs`.
...
1. **Decide name and scope.** Confirm `make-docs` on the public registry, or switch to a scoped name. Update `packages/cli/package.json#name` if needed.
...
5. **Bump version** to `1.0.0-rc.1` via `npm version --no-git-tag-version 1.0.0-rc.1 -w make-docs` (git tagging is deferred until after publish).
...
8. **Dry run once more.** Run `npm pack --dry-run -w make-docs` and inspect the file list one final time.
9. **Publish the rc**:
   ```bash
   npm publish --access public --tag next -w make-docs
   ```
...
10. **Verify from a clean directory**:
    ...
    npx make-docs@next init --yes
    ls docs/
    cat docs/.make-docs/manifest.json | head
    ```
...
- **Option A — promote the existing rc.** Run `npm dist-tag add make-docs@1.0.0-rc.1 latest`. The same artifact becomes the default install.
- **Option B — publish a fresh `1.0.0`.** Bump to `1.0.0`, re-run the validation chain, then `npm publish --access public -w make-docs` (omitting `--tag` defaults to `latest`).
...
- **72-hour unpublish window.** `npm unpublish make-docs@<version>` works only within 72 hours of publish. After that, only deprecation is available.
- **Deprecation.** `npm deprecate make-docs@<version> "reason"` leaves the version installable but warns users. Used if a published version turns out to be broken.
...
- **Unscoped `make-docs` vs scoped `@<user>/make-docs`.** Unscoped is preferred for discoverability and for a clean `npx make-docs` invocation; scoped is the fallback if the unscoped name is taken on the public registry.
...
**What becomes possible.** Consumers in other projects install the tool with `npx make-docs@next` (and, after promotion, `npx make-docs`). The Phase 7 tarball stops being a laboratory artifact and becomes a shipped release.

**Risks.**

- Files outside the intended allowlist leak into the published tarball via `package.json#files`.
- The package name `make-docs` is already taken on the public registry, forcing a last-minute switch to a scoped name.
...
1. **Package name** — stick with `make-docs` or switch to a scoped name such as `@<user>/make-docs`.
