...
Use the existing Store operation and installation services for intent, checkpoints, backup references, pending state, completion, and recovery. Preserve current shared payload roots and both native exposure methods in both scopes. Clean managed upgrades must use bundled first-party bytes even when old records contain remote-source provenance. Edited managed files remain conflicts. Repeat and remove operations must use the resulting ownership, not filename assumptions.
...
| Exposure and scope | Local and global scope; Codex and Claude; supported symlink and copy exposure; none remains empty. | P1/P2; PRD 28. |
...
| Lifecycle and recovery | Ownership-only adoption, repeat, managed update, edited-file conflict, removal, old remote-source upgrade without fetch, interruption at mutation boundaries, unavailable Store, resume/verify through existing recovery. | P3; PRDs 08, 28, 38. |
