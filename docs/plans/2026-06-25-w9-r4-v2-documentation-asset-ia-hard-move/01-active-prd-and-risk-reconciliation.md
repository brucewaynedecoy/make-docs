...
- Update [PRD 21](../../prd/21-project-tool-directory-and-resource-tiers.md) so `.make-docs/**` owns contracts, references, scripts, templates, agentics, config, manifest, runtime provenance, and instruction routers without keeping a separate future top-level prompt family unless a later design reintroduces one.
- Update [PRD 22](../../prd/22-project-documentation-asset-model.md#requirements) so `docs/assets/**` contains `archive`, `artifacts`, `breadcrumbs`, `guides`, and `playbooks`.
