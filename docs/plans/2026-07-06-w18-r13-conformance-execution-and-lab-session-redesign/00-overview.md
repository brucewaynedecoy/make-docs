...
| PRD 37 Enhance Playbook and Package Conformance | change PRD | [../../prd/20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md#support-claim-governance) | High — the primary baseline being revised: R-SCEN scenario identity and absence reporting, the Contracts and Data raw-evidence clause, and the R-TEST meta-verification family gaining the executability check |
| PRD 20 Revise Agent Harness Model Conformance Lab | change PRD | [../../prd/20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md) | High — the lab baseline whose raw-artifact default (`.make-docs/conformance/` or `.make-docs/runs/conformance/`) is superseded and whose execution protocol gains the lab-session vocabulary, the three operator modes, and the agent-driven rule |
| PRD 42 Revise Conformance Asset Home Relocation | change PRD | [../../prd/43-conformance-scenario-model-and-execution-kits.md](../../prd/43-conformance-scenario-model-and-execution-kits.md#canonical-conformance-asset-home) | High — repo-root `conformance/` home explicitly preserved (D-022); its Effective Requirement restates the now-rejected raw-transcript default and needs a scoped annotation |
...
- New change docs:
  ...
  - `docs/prd/44-conformance-lab-sessions-and-evidence.md`
- Baseline docs to annotate: `docs/prd/20-agent-harness-conformance-and-support-claims.md`, `docs/prd/20-agent-harness-conformance-and-support-claims.md`, `docs/prd/43-conformance-scenario-model-and-execution-kits.md`, `docs/prd/36-playbook-packaging-compiler-and-harness-adapters.md`
...
| ------- | ---- | ------------- | ---------------------- |
| [43-conformance-scenario-model-and-execution-kits.md](../../prd/43-conformance-scenario-model-and-execution-kits.md) | revision | Supersedes the harness-named `codex-*` scenario identity, the `futureHarnesses` property, and the authored-executability spec form with domain-qualified harness-agnostic definitions under `conformance/scenarios/<domain>/`, `targets` bindings, results by execution target, the generated per-target kit with deterministic instruments and prompts, the discovery kit, fail-closed ingestion into the unchanged recording seam, and the executable-by-construction requirement closing D-023. | PRD 37, PRD 36, plus the PRD index. |
| [44-conformance-lab-sessions-and-evidence.md](../../prd/44-conformance-lab-sessions-and-evidence.md) | revision | Supersedes the lab's repo-local raw-evidence default with the disposable lab-session workspace plus the machine-level store's lab area (D-024), and revises the lab's execution protocol: agent-driven self-assessment as the primary mode under "the agent drives, the instruments measure" (R-EXEC-1..3, self-attestation prohibited), three first-class operator modes, and the lab-session vocabulary that un-overloads "run". | PRD 20, PRD 42, plus the PRD index. |

## Baseline Annotation Plan

| Baseline doc | Impacted sections | Note verb | Target change doc |
| ------------ | ----------------- | --------- | ----------------- |
| [20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md#support-claim-governance) | Required First-Pass Scenarios (R-SCEN), appended newest-last to the existing `#### Change Notes` block | Superseded by | [43-conformance-scenario-model-and-execution-kits.md](../../prd/43-conformance-scenario-model-and-execution-kits.md) |
| [20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md#support-claim-governance) | Verification and Meta-Verification (R-TEST) as a new `#### Change Notes` block | Enhanced by | [43-conformance-scenario-model-and-execution-kits.md](../../prd/43-conformance-scenario-model-and-execution-kits.md) |
| [20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md#support-claim-governance) | Contracts and Data, appended newest-last to the existing `### Change Notes` block | Superseded by | [43](../../prd/43-conformance-scenario-model-and-execution-kits.md) (scenario/result organization) and [44](../../prd/44-conformance-lab-sessions-and-evidence.md) (raw-evidence default) |
| [20-agent-harness-conformance-and-support-claims.md](../../prd/20-agent-harness-conformance-and-support-claims.md) | Effective Requirement, appended newest-last to the existing `### Change Notes` block | Superseded by | [44-conformance-lab-sessions-and-evidence.md](../../prd/44-conformance-lab-sessions-and-evidence.md) |
| [43-conformance-scenario-model-and-execution-kits.md](../../prd/43-conformance-scenario-model-and-execution-kits.md#canonical-conformance-asset-home) | Effective Requirement as a new `### Change Notes` block, scoped to the raw-transcripts sentence only; the repo-root `conformance/` home is explicitly preserved | Superseded by | [44-conformance-lab-sessions-and-evidence.md](../../prd/44-conformance-lab-sessions-and-evidence.md) |
...
| ------ | ----- | ----------- | ------------ | ------------ |
| Scenario-model change doc author | Author PRD 43 from the revision template carrying the design's R-ORG, R-SCHEMA, R-KIT, R-HOME, R-INST, R-PROMPT, R-DISC, and R-ING requirement families plus the D14 verification bar and the D12 reconciliation obligation | `docs/prd/43-conformance-scenario-model-and-execution-kits.md` | Accepted W18 R13 design | The scenario-model and execution-kit effective-requirement change doc. |
| Lab-protocol change doc author | Author PRD 44 from the revision template carrying the design's R-EXEC, R-MODE, and R-NAME requirement families | `docs/prd/44-conformance-lab-sessions-and-evidence.md` | Accepted W18 R13 design | The execution-protocol and evidence-homes effective-requirement change doc. |
