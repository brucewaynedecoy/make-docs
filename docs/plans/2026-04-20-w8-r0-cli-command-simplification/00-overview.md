...
- Bare `starter-docs [options]` becomes the primary idempotent apply/sync command for both first install and existing installs.
- `starter-docs reconfigure [options]` becomes the explicit selection-change command for existing installs.
...
1. `starter-docs --help`, `starter-docs reconfigure --help`, `starter-docs backup --help`, and `starter-docs uninstall --help` render the new command surface.
2. `starter-docs init`, `starter-docs update`, and `--reconfigure` fail with helpful migration guidance.
...
4. Generated router text no longer tells users to rerun `npx starter-docs update --reconfigure`.
5. `npm run build -w starter-docs`, `npm test -w starter-docs`, and `node scripts/smoke-pack.mjs` pass.
