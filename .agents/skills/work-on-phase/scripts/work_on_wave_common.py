#!/usr/bin/env python3
"""Shared helpers for the work-on-phase skill scripts."""
...
def resolve_target(target: str, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if coord.group(3) is None:
        raise WaveError(
            "work-on-phase requires an explicit phase target such as `W14 R2 P1` "
            "or a direct docs/work phase file path."
        )
    phase = int(coord.group(3))
...
def resolve_path(path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if path.is_dir():
        raise WaveError(
            "work-on-phase requires a direct docs/work phase file path; "
            f"directory targets are ambiguous: {path}"
        )
    raise WaveError(f"Path does not resolve to a phase file: {path}")
