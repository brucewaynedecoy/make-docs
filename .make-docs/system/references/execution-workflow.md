...
- In each generated phase, record the current decision for Automated Implementation Testing, Performance Testing, Guided Progress Review, and Unassisted Goal Testing. Record `not-needed-now` when a type cannot change a current decision. Keep Human Experience Review separate as required agent review work for applicable promises, not a fifth testing type. Have the agent inspect the available real result and record conclusions and limits. For completed direct human-facing work, normally include a short optional experience handoff. Require a human response only when accepted authority explicitly defines a human acceptance gate.
...
8. Confirm each generated phase records all four current testing decisions and keeps Human Experience Review separate as the agent-owned review lens. Confirm that the review records evidence, observations, conclusions, reviewer, and limits. Confirm that an optional experience handoff is not treated as a gate and that any human acceptance gate has explicit authority, scope, reviewer, surface, question, and gate effect.
9. If the reply asks the user to act, confirm that the request is understandable without opening another document. State the action, reason, whether it is required, optional, or a separate authorization, the work it blocks, where supporting detail is available, and what happens after the action. Explain needed internal terms and identifiers.
