...
| Review | Inspect the actual human-facing surface for `direct` impact when it exists. Tie `indirect` evidence to the stated human effect. The agent prepares and presents a concise review with proposed per-promise observations, conclusions, limits, and next actions. |
| Coverage | Apply Human Experience Review as a lens over suitable evidence for each applicable promise. Reuse evidence that answers the promise. Ask for more input only when evidence is insufficient or the owner identifies a gap. Human Experience Review is not a separate testing type. |
| Acceptance | Record the owner's concise response against each promise and then record the approved structured review. An owner correction replaces the affected proposed conclusion. The owner does not need to write a formal report or use a special phrase. Keep an accepted material gap open or route later owed work through an accepted obligation. |
