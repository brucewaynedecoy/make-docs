...
This backlog converts the active `make-docs` PRD set into dependency-ordered implementation work. The phase order follows the live architecture captured in [../../prd/01-product-overview.md](../../../../docs/prd/01-product-overview.md) and [../../prd/02-architecture-overview.md](../../../../docs/prd/02-architecture-overview.md): typed install state and manifest safety in `packages/cli/src/types.ts:38-271`, `packages/cli/src/profile.ts:10-99`, and `packages/cli/src/manifest.ts:18-245` come first; asset selection and rendering in `packages/cli/src/catalog.ts:64-85`, `packages/cli/src/rules.ts:130-194`, and `packages/cli/src/renderers.ts:40-570` come next; public CLI and skills behavior in `packages/cli/src/cli.ts:77-244` and `packages/cli/src/skill-catalog.ts:33-138` follow; dogfood, packaging, and release validation land last through `packages/cli/src/utils.ts:33-55`, `packages/cli/package.json:9-25`, and `scripts/smoke-pack.mjs:60-246`.
...
- This backlog now incorporates the full `docs/prd/01` through `docs/prd/10` set. The fixed-core overview docs [../../prd/01-product-overview.md](../../../../docs/prd/01-product-overview.md) and [../../prd/02-architecture-overview.md](../../../../docs/prd/02-architecture-overview.md) reinforce the same dependency ordering already visible in the subsystem docs rather than changing it.
...
- Treat `packages/content`, skills delivery mode, and template/reference mode semantics as explicit decision points from [../../prd/03-open-questions-and-risk-register.md](../../../../docs/prd/03-open-questions-and-risk-register.md). Do not leave them as implied future work.
