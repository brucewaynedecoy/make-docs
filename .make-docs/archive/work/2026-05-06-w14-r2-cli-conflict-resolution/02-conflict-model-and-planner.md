...
- [docs/prd/07-cli-command-surface-and-lifecycle.md](../../../../docs/prd/07-cli-command-surface-and-lifecycle.md)
...
- [x] t1: Replace or rename `InstructionConflictResolution` and `InstructionConflictResolutions` in `packages/cli/src/types.ts`.
- [x] t2: Define overwrite/skip-only resolution types for reviewable managed-file diffs.
- [x] t3: Replace or rename `InstructionConflict` with a type that includes relative path, group, source id, reason, and optional instruction kind.
...
- [x] t4: Add classification for root and router `AGENTS.md` / `CLAUDE.md` files as agent instructions.
- [x] t5: Add classification for managed `docs/assets/references/` files as references.
- [x] t6: Add classification for managed `docs/assets/templates/` files as templates.
- [x] t7: Expose reviewable diffs from install planning without changing unrelated `skip-conflict` behavior.
...
- [x] t8: Update `packages/cli/src/planner.ts` so `overwrite` maps to normal `update` or `generate` actions with desired content.
- [x] t9: Update `packages/cli/src/planner.ts` so `skip` remains a `skip-conflict` action with a clear reason.
- [x] t10: Remove append-merge behavior from the active conflict-resolution path.
- [x] t11: Update `packages/cli/src/install.ts` and downstream imports for the generalized conflict discovery API.
...
- [x] t12: Search for stale instruction-only conflict names after the refactor.
- [x] t13: Run focused TypeScript type checks or tests that exercise `packages/cli/src/types.ts`, `packages/cli/src/planner.ts`, and `packages/cli/src/install.ts`.
- [x] t14: Reindex code with `jcodemunch` if needed for follow-on implementation review.
