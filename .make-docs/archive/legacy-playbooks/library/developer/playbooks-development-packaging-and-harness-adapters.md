...
related:
  ...
  - "../../../../../docs/work/2026-07-01-w18-r8-playbook-packaging-compiler-and-harness-adapters/00-index.md"
---
...
Since W18 R12 Phase 3, the packaging CLI grammar is intent-named per [PRD 39](../../../../../docs/prd/39-cli-command-model-and-operation-registry.md#package-grammar-and-ship-r-gram) R-GRAM-1..3, under the round's governing agent-invariance rule (R-INV-1): operation results, MCP tool output, and the machine-readable CLI output stay byte-identical to the pre-remediation shapes except for additive fields and flags. Two distinct mechanisms realize the grammar, and extenders must keep them distinct.
...
| --- | --- | --- | --- |
| `codex` | `verified` | The design's D6/R-ADAPT-2 verified-contract statement plus Section 8 of [the playbook architecture artifact](../../../../../docs/assets/project/playbook-architecture.md); a residual provisional note flags that MCP-server hosting inside the plugin container remains inferred. | `sha256:2033146ce7fa5b71` |
...
Since W18 R9 Phase 1, the registry end of that promotion path exists. `packages/cli/src/conformance/tuple.ts` extends the seven-dimension claim tuple into the eight-field conformance tuple (PRD 20 R-TUPLE-1) by binding one added dimension, `generatedOutputKind` — the ownership-record kind of what was actually produced, as distinct from the requested `outputKind`: `bindConformanceSupportTuple` consumes the claim tuple as-is, refuses an unresolved `auto` surface as a claim broader than any evidence, and a parity test pins the dimension relationship so the two lineages cannot drift apart silently. `conformance/tuple-registry.json`, loaded fail-closed by `packages/cli/src/conformance/registry.ts`, is the queryable home of tuple statuses: statuses are derived from recorded evidence and a stored status the evidence does not support fails the load, so a claim structurally cannot outrun its evidence at the registry layer either. The registry is seeded with the twenty first-party descriptor-placement tuples at zero `conformance-validated`, matching the provisional caps above. Since W18 R9 Phase 2 the scenario end of the promotion path also exists: the four first-pass scenario definitions — since W18 R13 Phase 1 harness-agnostic and domain-organized under `conformance/scenarios/packaging/` with per-target bindings, Codex bound first ([PRD 43](../../../../../docs/prd/43-conformance-scenario-model-and-execution-kits.md)) — implement the install-discover-invoke-uninstall evidence bar as the scenario shape, and `recordConformanceRunOnRegistryEntry` is the single seam that binds a recorded run onto a registry tuple — no scenario has run yet, so every status is unchanged. The tuple contracts, registry drift-proofing, derivation rules, seed posture, and scenario contracts are documented in [Conformance Lab Scenario and Result Contracts](../../../../../docs/assets/maintainer/conformance-lab-scenario-and-result-contracts.md).

<!-- support-claim-state: conformance-validated=0/20 -->

Since W18 R9 Phase 4, the promotion is also governed at the wording layer (PRD 20 R-GOV-1..2): a public claim states only what a `conformance-validated` tuple proves, and until then wording distinguishes a Make Docs generated output from a harness-recognized plugin. `packages/cli/src/conformance/governance.ts` adds a third, maintainer-side cap composing with the two above — `capSupportStatusForConformanceRegistry` holds any `validated` claim at `provisional` unless the exact registry tuple is `conformance-validated` — and `listPackagingSupportRegistryAgreementErrors` proves the wiring both ways: every first-party descriptor placement claim (the W18 R5/PRD 36 generated-output claims and this lineage's R-ADAPT-1/R-PROV-3 adapter support statuses) has exactly one registry tuple as its promotion path, and every registry tuple anchors back to a placement, so no parallel or prose-only support surface exists (R-REG-1). Public wording itself is derived, review-gated (one reviewed qualifying run for nominal wording, repeated reviewed runs for stronger wording), and bound to this guide through a machine-checked `support-claim-state` marker: when a tuple advances, the governance suite fails until this guide's support wording is reviewed and re-marked in the same change. See Support-Claim Governance in [Conformance Lab Scenario and Result Contracts](../../../../../docs/assets/maintainer/conformance-lab-scenario-and-result-contracts.md).
...
| `skill`, `plugin` | Harness-native manifest reference — the reference identifier is the resolved `probe` (PRD 34 R-DEP-2) — where the selected container carries a manifest that can host the primitive; explicit declared degradation in the skill text where it cannot (R-CAP-4). |
...
Current lifecycle coverage lives in `packages/cli/tests/playbook-packaging-lifecycle.test.ts` (R-PROV-2). It runs the real `runUninstallCommand` against actually written Codex plugin and Claude Code skills-bundle packages: backup copies exactly the audited owned outputs; uninstall removes the canonical payload and the exposure, prunes emptied `.codex`/`.agents` directories without orphaning managed directories, and preserves user-authored files and locally modified generated files. The compiled `conformance.json` carries a `cleanlinessScenario` reference to the PRD 43-owned backup/uninstall cleanliness scenario — referenced, not implemented here. Since W18 R9 Phase 2 that referenced scenario exists as a committed definition — `packaging/uninstall-backup-cleanliness` at `conformance/scenarios/packaging/uninstall-backup-cleanliness.json` since the W18 R13 Phase 1 reorganization — whose Codex target binding owns the real-harness half of R-PROV-2, asserting backup and removal against a real Codex install with orphan-directory and user-authored-sentinel assertions; the lifecycle tests here remain the internal file-and-structure layer, never harness-recognition evidence (R-TEST-5), and the scenario has not run yet — see [Conformance Lab Scenario and Result Contracts](../../../../../docs/assets/maintainer/conformance-lab-scenario-and-result-contracts.md).
...
- [Conformance Lab Scenario and Result Contracts](../../../../../docs/assets/maintainer/conformance-lab-scenario-and-result-contracts.md)
...
- [Playbook Packaging Compiler and Harness Adapters](../../../../../docs/designs/2026-07-01-playbook-packaging-compiler-and-harness-adapters.md)
- [36 Playbook Packaging Compiler and Harness Adapters](../../../../../docs/prd/36-playbook-packaging-compiler-and-harness-adapters.md)
- [36 Revise Playbook Packaging Compiler and Harness Adapters](../../../../../docs/prd/36-playbook-packaging-compiler-and-harness-adapters.md)
- [34 Playbook Authoring Contract and Model](../../../../../docs/prd/34-playbook-authoring-contract-and-model.md)
- [39 CLI Command Model and Operation Registry](../../../../../docs/prd/39-cli-command-model-and-operation-registry.md#package-grammar-and-ship-r-gram)
- [43 Revise Conformance Scenario Model and Execution Kit](../../../../../docs/prd/43-conformance-scenario-model-and-execution-kits.md)
- [W18 R5 Work Backlog](../../../../../docs/work/2026-06-29-w18-r5-playbook-packaging-and-harness-adapter-registry/00-index.md)
- [W18 R8 Work Backlog](../../../../../docs/work/2026-07-01-w18-r8-playbook-packaging-compiler-and-harness-adapters/00-index.md)
