...
related:
  ...
  - "../../../../../docs/assets/maintainer/cli-mcp-operation-parity-and-permissions.md"
  - "../../../../../docs/assets/maintainer/development-workflows-stage-model-and-artifact-relationships.md"
  ...
  - "../../../../../docs/work/2026-06-29-w18-r5-playbook-packaging-and-harness-adapter-registry/00-index.md"
  - "../../../../../docs/work/2026-07-01-w18-r7-run-playbook-state-machine/00-index.md"
---
...
Accepted shipped Playbook defaults use the PRD 06 source-of-truth path:
...
## CLI Render Layer and Ergonomics

Since W18 R12 Phase 3, the `run` dispatcher carries a render layer at the seam where `printJson(invocation.value)` previously sat ([PRD 39](../../../../../docs/prd/39-cli-command-model-and-operation-registry.md#human-experience-and-package-grammar) R-RENDER-1..3), implemented in `packages/cli/src/run/render.ts` and keyed by the previously unused `OperationRenderMode` in `packages/cli/src/operations/types.ts` — the presentation seam the source lineage anticipated but never built. Mode selection is the whole contract: `--json` always emits the full operation result, a non-TTY stdout defaults to the same full JSON so pipes, scripts, and agents observe no change without passing any flag, and only a TTY defaults to `text`. The agent-invariance proof (R-INV-1, R-TEST-4) lives in `packages/cli/tests/run-cli-experience.test.ts`, which captures both channels and asserts them byte-identical (`toBe` string assertions, not deep-equal) to the pre-render-layer `printJson` output; because vitest's captured stdout is never a TTY, `runRunCommand` takes an injectable `isTty` seam that tests use to force text mode, while production reads `process.stdout.isTTY`.

Text mode renders per operation family. Run-state transitions (`start`, `advance`, `gate`, `resume`, `close`, `status`) print what just happened (the advance execution report — step, mode, action, outcome, presented command or instructions), where the run stands (one compact run/cursor/status line, never the full state echo), the run's current guidance hints, a one-line full-record reference, and the exact next command as the last line; `playbook.next` renders the position report with its own next command; `playbook.catalog` renders the entry list; and the package pipeline operations print their human-oriented `lines` with a status header and a next-command handoff. Every printed command is derived from the registry's `operationCliPath` — never a hand-maintained string — so rendered guidance can only name commands the CLI accepts. Per R-RENDER-2 (UAT X7), the capability snapshot renders once, in the `start` text, and every later rendering references `--json`/`status --json` for the full record instead of restating it; the evidence log is never echoed in text mode. Rendering is deliberately defensive: renderers read the result value as plain JSON with no schema, any shape surprise or renderer throw falls back to the full JSON dump, and an operation without a bespoke renderer falls back to a generic `lines` renderer or the JSON dump — a renderer can never make an operation fail or lose data. The layer is CLI-only; MCP output derives from the operation result exactly as before (R-RENDER-3), in the surface presentation responsibility PRD 39 R-CORE-1 already assigns. Extenders adding an operation with TTY output add a renderer to `TEXT_RENDERERS` in `render.ts`, keyed by the spelled CLI identifier (so `package.preview` renders as preview, not write); never let text mode alter what the `--json` or non-TTY channels emit.

Run-id ergonomics (R-RUNID-1) live in `resolvePlaybookRunIdSelector` in `run-state.ts`, applied by the CLI adapters to every operation that addresses an existing run — `status`, `next`, `advance`, `gate`, `resume`, `close`, and `run.export` — while the minting operations (`start`, `invoke`) keep their plain `--run-id` value. The semantics: an exact run id always wins, even when it is also a prefix of others; an unambiguous prefix resolves to the single match; an ambiguous prefix fails listing every candidate; and `--last` selects the project's most recent run (latest `createdAt`, run id as the deterministic tie-break) and cannot be combined with `--run-id`. Two recorded implementer decisions keep the selector invariance-safe: a prefix with no match passes through unchanged so the operation's own unknown-run error stays byte-identical, and any store or manifest failure during prefix listing also falls back to the raw value — resolution never introduces a new failure mode for an invocation that worked before (R-INV-1). `--last` has no pass-through, because it cannot resolve without the store. New `--run-id` acceptors must route through `resolveRunIdOption` in `run/cli.ts` rather than reading the flag directly.

Two more Phase 3 ergonomics ride the same surface. `--repo-root` defaults to the nearest ancestor of the working directory carrying `.make-docs/manifest.json`, falling back to the working directory itself, with the flag kept as an override (R-FLAG-1); `--store-root` keeps its existing explicit-option → `MAKE_DOCS_HOME` → `~/.make-docs` resolution. And the Node SQLite ExperimentalWarning noise is gone (R-NOISE-1): `packages/cli/src/run/warnings.ts` wraps `process.emitWarning` at the bin entry (`src/index.ts`) with a filter matching only the `ExperimentalWarning` type AND the SQLite message text — every other process warning, including other experimental warnings, still surfaces, and tests cover both the suppression and the pass-through plus the idempotent install/uninstall seam. Never widen this to a blanket suppression. All of the above is covered by `packages/cli/tests/run-cli-experience.test.ts`; the packaging-grammar half of Phase 3 (`preview`/`write`/`ship`, the spelling table, and precondition config absorption) is documented in [Playbook Packaging and Harness Adapters](playbooks-development-packaging-and-harness-adapters.md).
...
- [CLI/MCP Operation Parity and Permissions](../../../../../docs/assets/maintainer/cli-mcp-operation-parity-and-permissions.md)
- [Understanding the Make Docs Stage Model](../../../../../docs/assets/maintainer/development-workflows-stage-model-and-artifact-relationships.md)
- [25 Revise CLI Separation and MCP Boundary](../../../../../docs/prd/25-typescript-runtime-cli-mcp-operation-boundaries.md)
- [35 Run Playbook State Machine and Portability](../../../../../docs/prd/35-run-playbook-state-machine-and-portability.md#requirements)
- [30 Revise Harness Plugin Substrate Workflow Bundles](../../../../../docs/prd/30-plugin-substrate-and-workflow-bundles.md)
- [36 Playbook Packaging Compiler and Harness Adapters](../../../../../docs/prd/36-playbook-packaging-compiler-and-harness-adapters.md)
- [34 Revise Playbook Contract and Model](../../../../../docs/prd/34-playbook-authoring-contract-and-model.md)
- [35 Revise Run Playbook State Machine](../../../../../docs/prd/35-run-playbook-state-machine-and-portability.md)
- [38 Revise Global Store and Project State](../../../../../docs/prd/38-global-store-and-project-state.md)
- [34 Playbook Authoring Contract and Model](../../../../../docs/prd/34-playbook-authoring-contract-and-model.md)
- [39 CLI Command Model and Operation Registry](../../../../../docs/prd/39-cli-command-model-and-operation-registry.md#human-experience-and-package-grammar)
...
- [Run Playbook Orchestration and Harness Capabilities](../../../../../docs/designs/2026-06-27-run-playbook-orchestration-and-harness-capabilities.md)
- [Playbook Packaging and Harness Adapter Registry](../../../../../docs/designs/2026-06-29-playbook-packaging-and-harness-adapter-registry.md)
